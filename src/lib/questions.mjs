...
export function loadQuestions() {
  ...
  return entries.map(({ id, question }, index) => {
    ...
    return { ...data, number: index + 1, sections: sections.map((text) => marked.parse(text)) };
  });
}
...
export function relatedQuestions(question) {
  const explicit = question.related.map((id) => questions.find((item) => item.id === id));
  const inferred = questions
    .filter((item) => item.id !== question.id && !question.related.includes(item.id))
