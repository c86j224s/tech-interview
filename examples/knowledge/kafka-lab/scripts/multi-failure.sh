...
python3 tests/poll_replication.py .lab-inbox/topic-before.txt 3 3 --file compose.multi.yaml --service kafka-1
KAFKA_BOOTSTRAP="$BOOTSTRAP" "$JAVA_BIN" -cp "$CP" lab.kafka.ProducerMain 6
set +e
INBOX_FILE="$INBOX_FILE" KAFKA_BOOTSTRAP="$BOOTSTRAP" STOP_AFTER_EVENT_ID=evt-2 KILL_BEFORE_COMMIT=true MAX_RECORDS=3 "$JAVA_BIN" -cp "$CP" lab.kafka.InboxWorker > .lab-inbox/consumer-kill.log 2>&1
status=$?
set -e
test "$status" -eq 42
python3 tests/assert_consumer.py .lab-inbox/consumer-kill.log killed
test "$(python3 tests/count_inbox.py "$INBOX_FILE")" -eq 3
$COMPOSE -f compose.multi.yaml stop kafka-1
python3 tests/poll_replication.py .lab-inbox/topic-down.txt 3 2 1 --file compose.multi.yaml --service kafka-2
KAFKA_BOOTSTRAP="$BOOTSTRAP" INBOX_FILE="$INBOX_FILE" MAX_RECORDS=6 "$JAVA_BIN" -cp "$CP" lab.kafka.InboxWorker > .lab-inbox/consumer-restart-down.log 2>&1
python3 tests/assert_consumer.py .lab-inbox/consumer-restart-down.log restarted
test "$(python3 tests/count_inbox.py "$INBOX_FILE")" -eq 6
$COMPOSE -f compose.multi.yaml start kafka-1
python3 tests/poll_replication.py .lab-inbox/topic-after.txt 3 3 --file compose.multi.yaml --service kafka-2
