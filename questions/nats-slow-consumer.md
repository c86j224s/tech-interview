...
큐 길이만 보지 않고 가장 오래된 메시지의 나이, 유실·재전달, callback 지연, 메모리를 함께 경보로 삼겠습니다. 소비자를 의도적으로 느리게 하고 연결 해제·회복·부하 증가를 시험해 무한 큐가 메모리와 tail latency를 숨기지 않는지, 확장이 중복 업무를 만들지 않는지 확인합니다.
