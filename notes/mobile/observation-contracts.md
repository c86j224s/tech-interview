...
iOS의 delegate, 완료 클로저, Notification은 문법 선택지가 아니라 결과의 소유권과 관찰 수명에 관한 서로 다른 계약입니다. 이 노트는 현재 상태 snapshot과 과거 사건을 나누고, 등록·초기 상태 설정·해제 사이에서 사건이 빠지거나 중복되지 않게 만드는 경계를 설명합니다.
...
상태 변경을 snapshot으로 보낼지 delta로 보낼지 먼저 고정해야 버전 처리도 정해집니다. `snapshot v10` 뒤 `snapshot v11`은 늦은 v10을 버려도 되지만, `delta v11`은 v10에서 적용해야 하므로 누락 감지와 원본 재조회가 필요합니다. 같은 버전 숫자라도 payload 의미가 다르면 같은 병합 규칙을 적용할 수 없습니다.
...
## 구독 등록 전후 상태 변경 재현
재현 로그에는 등록 시각, 초기 snapshot 버전, 이벤트 버전, 해제 토큰, UI가 실제로 적용한 마지막 버전을 함께 남깁니다. 이 기록이 있으면 화면이 오래된 상태를 그린 원인이 사건 누락인지, 순서 역전인지, 중복 등록인지 빠르게 구분할 수 있습니다.
