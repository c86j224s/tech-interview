...
UIView의 좌표 문제는 숫자 계산보다 “그 숫자가 어느 뷰의 공간에 있는가”를 먼저 정하는 문제입니다. `bounds`의 내부 기준을 `frame`의 부모 공간과 섞지 않고, 중첩 스크롤·transform·애니메이션에서는 변환 대상과 측정 시점을 명시해야 터치 판정과 경계 계산이 같은 위치를 가리킵니다.
...
`frame=(20,30,100,40)`, `bounds.origin=(10,0)`인 뷰에서 로컬 점 `(10,7)`은 부모의 `(20,37)`에 대응합니다. 이를 frame에 그대로 더하면 `(30,37)`이 되어 틀리므로, 실제 대응식은 `frame.origin + (local - bounds.origin)`입니다. 따라서 로컬 점 `(0,7)`은 부모의 `(10,37)`에 해당합니다. transform이나 scroll이 추가되면 이 수식을 계층 전체에 손으로 확장하지 않고 `convert`의 왕복 결과로 검증합니다.
