...
부모 좌표에서 자식이 `(20,30,100,40)`에 놓였다고 합시다. 변환이 없는 단순한 경우 이것이 자식 frame입니다. 자식 내부에서 그리는 영역은 보통 bounds `(0,0,100,40)`입니다. 부모 배치와 자기 내부 좌표는 별개입니다.
...
위 자식의 bounds.origin이 (0,0)이면 내부 점 (5,7)은 부모에서 (25,37)입니다. bounds.origin이 (10,0)이면 내부 점 (10,7)이 부모의 (20,37)에 대응하는 식입니다. 단순 평행 이동 예이며 실제 anchor·transform·중첩 스크롤에서는 이런 수동 덧셈이 빠르게 복잡해집니다.
...
중심이 원점이고 폭 100·높이 40인 사각형을 45도 회전하면, 축 정렬 외접 폭은 `|100 cosθ|+|40 sinθ|`, 높이는 `|100 sinθ|+|40 cosθ|`입니다. θ=45도에서는 둘 다 약 98.995입니다. 원래 높이 40이 99에 가까워졌다고 내부 bounds의 높이가 그만큼 커진 것은 아닙니다.
...
터치를 받은 뷰의 점을 자식으로 보낼 때 `convert(_:to:)` 또는 `convert(_:from:)`처럼 대상 공간을 명시합니다. 각 부모 frame과 contentOffset을 수동 합산하면 bounds.origin·zoom·transform을 놓치기 쉽습니다. 같은 윈도우·뷰 계층의 지원 계약도 확인해야 합니다.
