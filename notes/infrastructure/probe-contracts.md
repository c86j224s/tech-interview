...
## HTTP와 gRPC Probe는 일반 Client의 모든 기능을 제공하지 않습니다
...
테스트 클러스터에서 느린 정상 초기화·영구 초기화 실패·필수 DB 지연·선택 의존 장애·로컬 진행 정지를 구분합니다. 정상 초기화 중 불필요한 restart가 없는지, Ready 이전에 트래픽이 들어오지 않는지, 재시작이 실제 회복에 기여하는지 확인합니다.
