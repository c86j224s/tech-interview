...
`prune`은 Application이 추적하는 리소스 집합을 렌더 결과와 비교한 뒤, 원하는 선언에서 빠진 객체를 sync 과정에서 삭제하는 동작입니다. 따라서 클러스터의 모든 객체를 지우는 것은 아니며, Git 파일 하나를 삭제했다고 반드시 객체 하나가 바로 삭제되는 것도 아닙니다. 실제로 무엇이 대상인지 보려면 렌더된 GVK·namespace·name, 리소스 추적 방식, 그리고 `resource identity`가 파일 경로와 달라지는 경우를 함께 대조합니다.
...
Application을 삭제할 때 관리 리소스까지 따라 지우는 cascading 범위는 Argo CD finalizer, 리소스 추적 방식, 삭제 옵션에 따라 달라집니다. 이것은 선언에서 빠진 한 리소스를 sync 중 정리하는 개별 prune과 계기와 범위가 같지 않으므로, Application 삭제 전에는 전체 추적 집합을 따로 펼쳐 봅니다. 그 뒤 Kubernetes owner reference와 외부 controller가 추가로 삭제할 수 있는 의존 객체까지 포함해 실제 연쇄를 검토합니다.
...
삭제 전에 PVC를 참조하는 Pod·Job·StatefulSet과 백업·관리 도구를 목록으로 만들고, 데이터 소유자가 요구하는 보존 기간과 복원 지점을 확인합니다. 그런 다음 PV의 `reclaimPolicy`, 스토리지 provisioner, StatefulSet PVC retention, snapshot deletion policy를 한 계층씩 따라가서 `Delete`일 때 underlying storage까지 삭제되는 경로가 있는지 봅니다.

`Retain`이어도 자동 재바인딩이나 복구가 보장되는 것은 아니므로, 남은 PV와 실제 backend ID를 복원 절차로 연결해 시험해야 합니다.

DB snapshot에는 필요한 페이지·WAL·키·설정이 일관된 집합으로 있어야 합니다. 별도 복원 환경에서 마지막 확인된 커밋·핵심 불변식·복구 시간을 검증합니다. 백업 생성 성공을 복구 완료라고 부르지 않습니다.

## 삭제 보호와 잔존 자원 정리의 동시 설계
