...
노드 교체를 안전하게 판단하려면 스케줄러가 Pod를 다른 곳에 놓을 수 있는지와 애플리케이션 상태가 중단을 견디는지를 분리해야 합니다. consolidation은 용량·비용 배치를, drift는 원하는 노드 구성과의 차이를 다루며, 어느 쪽도 메모리 세션을 자동 이전하지 않습니다.

## 낮은 사용률과 실행 중 Pod 중단의 안전성
...
## Consolidation과 Drift 교체의 목적 차이
...
## 자발적 교체와 알림 없는 장애의 분리 시험

정상 eviction에서는 새 Pod가 Ready가 된 뒤 연결 복구와 중복 효과를 확인하고, 강제 손실에서는 PDB가 있어도 snapshot·재접속·멱등 처리가 작동하는지 별도로 확인합니다. 두 결과를 같은 “무중단” 지표로 합치지 않아야 자발적 교체 보호와 장애 복구의 빈틈을 구분할 수 있습니다.
