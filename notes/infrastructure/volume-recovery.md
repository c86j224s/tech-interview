...
볼륨 복구는 연결 가능한 바이트를 다시 얻는 단계와 애플리케이션이 올바른 최신 상태로 안전하게 쓰기 시작하는 단계를 나눠야 합니다. PVC 상태, 영역 배치, 옛 writer 격리, DB 로그와 백업의 일관성은 서로 다른 증거이므로 하나의 Ready 신호로 합치지 않습니다.
...
복구 판정의 예는 다음과 같습니다. attach가 성공했지만 WAL replay가 마지막 확인 커밋보다 앞서 끝나지 않았으면 읽기만 허용할 수 있고, replay와 무결성 검사가 끝났어도 옛 writer fencing을 확인하지 못했으면 쓰기를 열지 않습니다. Ready는 이 조건들을 실제 health check가 증명할 때만 복구 완료 신호가 되어야 합니다.
