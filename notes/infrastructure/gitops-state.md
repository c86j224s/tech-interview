...
Git은 읽었지만 template 렌더가 실패할 수도 있고, 렌더된 리소스가 API 검증에서 거절될 수도 있으며, API 적용은 성공했지만 Pod readiness가 실패할 수도 있습니다. 이 각각을 단일 “배포 실패”로만 기록하면 고칠 경계가 불명확합니다. 저장소가 복구되면 현재 의도·긴급 변경 이력과 적용될 revision을 확인하고 무심코 낡은 선언을 되살리지 않습니다.
...
HPA가 replicas를 조정하고 GitOps가 고정 수를 계속 되돌리면 진동이 생깁니다. autoscaling 정책은 Git에서 관리하되 workload replica 필드의 실제 권위는 HPA에 두는 등 책임을 정합니다. metadata.managedFields·HPA·Application 정책을 함께 확인합니다.

ignoreDifferences로 diff만 숨기는 것과 sync 때도 그 필드를 덮지 않는 것은 다릅니다. RespectIgnoreDifferences 등 실제 적용 옵션과 대상이 이미 존재하는지의 조건을 확인합니다. 리소스 전체를 제외하면 image·권한·port drift까지 놓칠 수 있으므로 필요한 필드만 좁힙니다.
