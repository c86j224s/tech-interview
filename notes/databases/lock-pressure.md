...
이 노트는 대량 변경 때 함께 커지는 세 가지를 분리해 읽는 방법을 설명합니다. ‘몇 행을 바꿨는가’는 영향량이고, ‘어떤 자원을 얼마나 오래 잠갔는가’는 경합의 범위이며, ‘로그와 flush가 얼마나 발생했는가’는 내구·복구 자원입니다. 실행 계획과 엔진별 관측값을 연결한 뒤, batch 분할이 원자성·checkpoint 의미를 어떻게 바꾸는지까지 추적합니다.
...
재현 연습은 같은 입력을 넓은 scan과 검색 가능한 인덱스 경로로 각각 실행하고, blocker의 자원 종류·거래 시작 시각·보유 시간을 기록하는 것으로 시작합니다. 그 다음 외부 대기를 거래 안에 넣은 경우와 제거한 경우를 비교합니다. 예상 결과는 외부 대기 제거가 잠금 보유 시간은 줄여도 행당 로그 바이트를 자동으로 줄이지 않는 것이며, 1천 행 batch는 중간 commit과 부분 완료를 만들어 전체 원자성 확인 항목을 추가한다는 것입니다.
