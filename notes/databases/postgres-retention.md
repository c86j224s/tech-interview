...
MVCC에서 삭제·갱신된 옛 tuple을 이전 snapshot의 독자가 아직 필요로 할 수 있습니다. 더 이상 어떤 관련 독자에게도 필요 없을 때 정리할 수 있습니다. VACUUM은 이런 tuple과 관련 인덱스 정리·공간 재사용·visibility·freeze 관리에 관여합니다.

일반 VACUUM이 내부 공간을 재사용 가능하게 만드는 것과 테이블 파일을 OS에 크게 반환하는 것은 다릅니다. 끝의 비어 있는 페이지 처리 같은 조건부 축소가 가능해도 모든 bloat가 사라지는 것은 아닙니다. VACUUM FULL 같은 재작성은 더 강한 잠금·추가 공간·I/O 비용을 가진 별도 운영 작업입니다.
...
초당 20 MB WAL이 계속 생성되고 소비자가 한 시간 멈추면 단순 계산으로 72,000 MB가 생성됩니다. 실제 보존량은 정리 가능 위치·checkpoint·압축·다른 보존 요구에 달렸으므로 지표로 확인합니다. 남은 디스크 36,000 MB에 순증가 20 MB/s라면 약 30분이라는 대응 예산을 추정할 수 있지만 증가율 변화에 여유를 둡니다.

slot의 활성 여부·restart_lsn·확인 위치·실제 보존량·connector 처리율·가장 오래된 미적용 사건을 관찰합니다. 수신·처리·checkpoint 저장을 같은 완료로 보지 않습니다. 보존 상한을 넘겨 로그를 잃으면 connector가 옛 위치에서 그대로 재개하지 못하고 새 snapshot이 필요할 수 있습니다.
