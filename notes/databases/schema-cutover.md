...
이것은 버전·원본 값 조건의 설명용 예입니다. 백필 자체가 version을 올리는지·별도 migration version을 쓰는지·이미 올바른 새 값일 때 재실행 결과를 어떻게 보는지 실제 모델에서 정합니다. 사용자 쓰기와 전환 이력을 구분할 수 있어야 합니다.
...
checkpoint를 먼저 전진시키면 미완료 행을 영구 건너뛸 수 있습니다. 같은 transaction에 결과·진행 기록을 묶거나 재실행에 안전한 명시 상태를 둡니다. 계속 충돌하는 hot row는 별도 보류·재시도·최대 나이·수동 확인을 관리합니다. 동적으로 줄어드는 미처리 목록에 OFFSET을 적용하지 않고 안정된 키와 기준 범위를 사용합니다.
