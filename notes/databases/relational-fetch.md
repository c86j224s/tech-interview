...
관계형 조회의 병목은 SQL 한 문장의 속도만으로 설명되지 않습니다. 요청이 몇 번 왕복하는지, 한 행이 얼마나 반복되는지, 페이지의 단위가 부모인지 결합 행인지, 그리고 두 조회 사이 snapshot이 같은지를 함께 봐야 합니다.

## 개별 SQL 속도와 전체 요청 지연

주문 50개를 읽는 쿼리 하나 뒤 각 주문의 고객을 개별 조회하면 51번의 DB 호출이 생깁니다.

실패를 재현할 때 주문 50개와 고객 5명을 준비해 고객 ID가 반복되는 경우를 포함합니다. naive 경로는 주문 1회+고객 50회이고, 요청 loader 경로는 부모 조회 1회+고유 고객 ID batch 1회가 됩니다. 두 경로의 응답 값이 같아도 실제 SQL 수·반환 행·bytes·pool 점유가 다르므로, “쿼리 수가 줄었다”와 “p99가 개선됐다”를 같은 결론으로 쓰지 않습니다. 개별 실행이 짧아도 네트워크 왕복·풀 점유·파싱·결과 조합이 누적됩니다. 느린 쿼리 목록만 보지 말고 사용자 요청 trace의 SQL 수와 전체 DB 대기를 봅니다.
