...
키셋 페이지를 선택할 때 먼저 “연속 탐색인가, 임의 페이지 이동인가”와 “최신 데이터면 되는가, 보고서처럼 결과가 고정되어야 하는가”를 결정합니다. 전자는 안정된 전체 순서와 마지막 경계 키가 핵심이고, 후자는 snapshot 또는 저장된 결과 집합이 핵심입니다. 둘을 구분하지 않으면 OFFSET을 키셋으로 바꾸고도 사용자가 기대한 고정 결과나 페이지 점프를 제공하지 못합니다.

## OFFSET 깊은 페이지의 스캔 비용
...
시각과 ID는 NOT NULL이고 ID는 해당 범위에서 유일하다고 가정합니다. `time DESC, id DESC`에서 커서가 `(t,i)`라면 다음 행은 `time<t`이거나 `time=t`이면서 `id<i`인 행입니다. `time DESC, id ASC`에서는 같은 시각의 보조 키가 앞으로 커지므로 `id>i`를 써야 하며, 앞·뒤 페이지에서는 부등호와 읽는 정렬 방향을 함께 맞춥니다.
...
진단 시에는 페이지 1의 마지막 `(time,id)`와 페이지 2 조건을 로그로 함께 남기고, 페이지 사이에 정렬 키가 바뀌었는지·행이 삭제됐는지·읽기 replica가 바뀌었는지를 분리합니다. 중복은 경계 행을 잘못 포함했을 가능성이 크고, 누락은 수정 가능한 정렬 키 이동이나 snapshot 부재일 수 있어 같은 “페이지 버그”로 뭉뚱그리지 않습니다.

## 키셋 경계와 페이지 사이 데이터 변경의 비고정성

내림차순에서 이미 본 행의 `time`이 12에서 8로 바뀌면, 저장한 커서 뒤의 조건을 다시 만족해 다음 페이지에 재등장할 수 있습니다. 아직 안 본 행의 `time`이 8에서 12로 바뀌면 커서보다 앞쪽으로 이동해 다음 페이지에서 놓칠 수 있습니다. ID가 유일해도 수정 가능한 주 정렬 키가 페이지 경계를 가로질러 움직이는 문제는 남습니다.
...
cursor에는 마지막 키·필터 fingerprint·정렬 버전·tenant·필요한 사용자 scope·만료를 묶을 수 있습니다. 예를 들어 마지막 행이 `(time=10,id=2)`라면 이 값을 다음 페이지의 경계로 포함해 서명하거나 MAC할 수 있지만, 서명·MAC은 값이 바뀌었는지만 검출하고 내용을 숨기지는 않습니다. 민감한 키를 감춰야 한다면 암호화 또는 서버의 opaque 상태를 선택합니다.

서명이 유효한 옛 cursor를 받아도 서버는 사용자의 현재 접근권한을 다시 확인해야 합니다. 현재 scope가 사라졌거나 cursor의 필터·tenant가 요청과 다르면 재사용하지 못하게 하고, 길이·페이지 크기·형식·기한도 제한합니다. 최종 SQL에는 서버 인증 문맥을 계속 넣어 cursor가 쿼리 인가를 대신하지 않게 합니다.

## 기준 정렬과 전체 페이지 합집합 비교
