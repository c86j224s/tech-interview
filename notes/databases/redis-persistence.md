...
Redis 지속성을 판단하는 기본 모델은 메모리 상태, persistence 파일, 파일의 flush, 실제 복구를 서로 다른 사건으로 보는 것입니다. RDB와 AOF 중 하나를 “안전한 저장”으로 고르는 문제가 아니라, 허용 RPO와 재시작 시간, rewrite 중 자원 피크, 독립 백업의 복원 가능성을 같은 장애 시나리오에 대입하는 문제입니다.
...
선택 기준을 먼저 수치화하면 설명이 쉬워집니다. 마지막 성공 응답 뒤 어느 정도의 쓰기 손실을 허용하는지가 RPO이고, 복구에 허용되는 중단 시간이 RTO입니다. RDB는 snapshot 간격과 fork 시점이, AOF는 appendfsync·장치 flush와 rewrite 상태가 각각 이 경계를 바꾸므로, 설정 이름만 보고 보장 수준을 단정하지 않습니다.
...
진단 순서는 “어떤 파일을 읽었는가”, “그 파일의 기준 시점과 마지막 완전한 명령은 무엇인가”, “다른 장애 도메인에 복사·검증됐는가”, “원장과 대사했는가”입니다. 실험에서 프로세스 중단만 했으면 전원 손실 결과라고 부르지 않으며, Redis 8.10 이상의 BACKUP 계열을 언급할 때는 실행 중인 버전이 해당 명령을 지원하는지 먼저 확인합니다.
