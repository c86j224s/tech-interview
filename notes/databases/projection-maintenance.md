...
읽기 모델은 원본의 의미를 빠르게 조회하기 위한 파생 상태이지만, 모든 파생 상태가 같은 방식으로 갱신되지는 않습니다. 완전한 snapshot은 더 낮은 버전이 도착해도 교체하지 않을 수 있지만 delta는 순서와 누락을 보존해야 하므로, 먼저 데이터의 의미와 적용 단위를 구분해야 합니다.
...
판정 절차를 실제 상태로 적으면 `projectionVersion=12`인 완전 snapshot 뒤의 v11은 폐기하고, `lastSequence=12`인 delta 소비자가 v14를 받으면 v13을 기다리거나 재동기화해야 합니다. 같은 숫자 필드라도 “v12의 전체 total”과 “v12에 더할 +20”은 적용 규칙이 다르므로 이벤트 형식 자체를 schema에 드러내야 합니다.
...
재현 연습에서는 v12 전체 snapshot 뒤 v11 snapshot, v12 delta 뒤 v11 delta, delete 뒤 옛 update, rebuild 중 v13 변경을 각각 단독으로 넣습니다. 예상 결과는 완전 snapshot의 낡은 교체 거부, delta gap의 대기 또는 snapshot 재동기화, tombstone에 의한 부활 방지, 현재 세대가 바뀐 rebuild의 조건부 게시 실패입니다. 각 결과에서 최종 projection을 원본 재계산과 비교해야 “처리됨” 마커만 남은 경우를 잡을 수 있습니다.
