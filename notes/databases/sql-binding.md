...
SQL 요청은 데이터 값과 SQL 문법을 함께 전달하는 것이 아니라, 서버가 허용한 문장 구조에 데이터만 주입하는 과정으로 이해해야 합니다. 값 binding은 문법 혼입을 막지만, 허용되지 않은 식별자·과도한 패턴·다른 tenant 행 조회까지 자동으로 해결하지 않으므로 각 경계를 별도로 검증합니다.
...
재현 시험에서는 따옴표와 개행이 포함된 이름을 값으로 넣었을 때 반환 행과 SQL 구조가 변하지 않아야 하고, `recent` 같은 허용 별칭은 실행되지만 `password_hash` 같은 허용되지 않은 컬럼은 parameter로 우회되지 않아야 합니다. `%`와 `_`를 포함한 입력은 injection 여부와 별개로 wildcard로 해석될지 문자 그대로 검색할지를 예상 결과와 대조합니다.
