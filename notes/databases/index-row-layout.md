...
인덱스 성능은 탐색 연산 하나의 이름이 아니라 후보를 찾은 뒤 필요한 행과 버전을 얼마나 더 읽는지의 경로 비용입니다. 이 노트는 논리 키에서 물리 행, 반환 결과, 쓰기 유지 비용까지 한 요청을 따라갑니다.
...
고객 한 명이 1건을 갖는 경우와 10만 건을 갖는 경우를 나누어 실행 계획의 예상 행 수, 실제 행 수, lookup 횟수를 비교합니다. 같은 `Index Seek`라도 반환 행이 늘면 locator를 따라가는 비용과 네트워크 반환량이 달라지므로 선택도와 결과 규모를 함께 봐야 합니다.

## 엔진별 Locator와 행 접근 경로
...
한 요청의 추적은 `secondary(customer_id) → locator(PK/RID/tuple) → row/version → result`입니다. 이 중 locator가 키이면 두 번째 B-tree 탐색이 되고, 위치 정보이면 heap 페이지와 가시성을 확인할 수 있습니다. 엔진 표의 대표 모형을 실제 저장 형식 전체로 확대하지 않고 plan과 페이지 통계로 확인합니다.

## Covering 인덱스와 실제 Heap 접근 조건
...
PostgreSQL에서 동일 질의를 한 번은 오래 갱신되지 않은 페이지, 한 번은 최근 UPDATE한 페이지에서 실행하고 `Index Only Scan`과 `Heap Fetches`를 함께 기록합니다. 인덱스에 열이 있어도 visibility map이 all-visible이 아니면 heap fetch가 남을 수 있다는 예측을 직접 확인하는 연습입니다.

## Clustering key 폭과 보조 인덱스 비용
...
예를 들어 보조 인덱스 4개가 각각 8바이트 locator 대신 64바이트 locator를 저장하면 한 항목의 저장 폭뿐 아니라 페이지 fan-out과 캐시 적중 기회도 달라집니다. 이 계산은 비용 방향을 설명하는 모델이지 특정 엔진의 정확한 페이지 수가 아니며, 실제 migration 전에는 인덱스 크기·로그·쓰기 지연을 측정합니다.

## 순차 키와 마지막 페이지 latch 경합
...
동일한 동시 삽입률에서 순차 키와 임의 키를 비교할 때 page split 수만 보지 말고 마지막 페이지 대기, transaction lock, 로그량을 별도 계열로 수집합니다. 순차 키에서 latch 대기가 증가해도 그것이 행 잠금 경합이라는 뜻은 아니며, wait 분류가 원인 판정의 근거입니다.

## 결과 규모·쓰기 부하·인덱스 비용 비교
...
판정 순서는 (1) 대표 결과 cardinality와 실제 plan 고정, (2) lookup·heap fetch·읽은 페이지 측정, (3) covering 추가 뒤 쓰기 p99·인덱스 크기 측정, (4) 부하와 캐시 상태를 바꾼 재검증입니다. 이 환경에서는 세 엔진의 실측을 하지 않았으므로 숫자 개선을 주장하지 않습니다.
