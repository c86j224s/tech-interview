...
## Hash Tag는 함께 처리할 최소 단위를 한 슬롯에 둡니다

일반적으로 키의 CRC16 결과를 슬롯 수로 나눈 값을 사용하며, hash tag 규칙에 맞는 중괄호 부분이 있으면 그 부분을 해시합니다. 첫 `{` 뒤 첫 `}` 사이가 비어 있지 않아야 하며 빈 tag는 전체 키 해시로 처리되는 규칙을 확인합니다. 키 생성 코드는 cluster-aware client·서버 slot 검사로 대조합니다.
...
## 서로 다른 계정의 이체는 배치·원장 설계를 바꿉니다

계정 A와 B가 다른 슬롯이면 한 Lua로 두 잔액을 전역 원자 변경할 수 없습니다. 모두 한 tag에 넣으면 원자 범위를 넓힐 수 있지만 분산을 잃습니다. 실제 거래 단위별 공동 배치가 가능한지, 권위 DB에서 이체하고 Redis를 projection으로 둘지, 예약·보상 상태 머신을 사용할지 요구에 맞춰 선택합니다.

두 슬롯에 차례로 차감·증가하는 코드는 중간 중단에 부분 효과를 남깁니다. 계정별 멱등 key와 전역 논리 이체 상태·재처리가 필요하고, 보상도 실패할 수 있어 일반적인 단일 DB 거래와 같다고 부르지 않습니다. 원자 Lua도 런타임 오류의 일반 rollback이나 승격 보존을 자동 제공하지 않습니다.

## MOVED와 ASK는 라우팅 의미가 다릅니다
