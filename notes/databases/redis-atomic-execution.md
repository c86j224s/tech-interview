...
Redis에서 말하는 원자성은 요청 전체나 외부 업무가 한 덩어리로 성공한다는 뜻이 아닙니다. 이 노트는 재고 조건을 읽고 판단하고 변경하는 경계를 Redis 안팎으로 나누어, 각 기능이 막는 경쟁과 여전히 남는 재시도·대사 책임을 설명합니다.
...
## WATCH 충돌과 재시도 실패 처리
장애를 해석할 때 `EXEC`의 null, 명령별 오류 배열, 연결 끊김을 같은 “실패”로 기록하지 않습니다. null은 감시 충돌로 명령이 실행되지 않은 신호지만, 연결 끊김은 서버가 이미 실행했는지 모르는 상태일 수 있으므로 요청 ID 조회와 재고 대사를 먼저 수행해야 합니다.
...
실험 결과를 판정할 때는 반환 문자열만 보지 말고 재고 값, 요청 기록의 fingerprint·TTL, 두 번째 호출의 차감량을 함께 읽습니다. 특히 응답만 버린 뒤 재호출한 경우 `accepted`가 다시 반환되면서 수량은 한 번만 줄어드는지를 확인해야 멱등 기록의 효과를 검증할 수 있습니다.
