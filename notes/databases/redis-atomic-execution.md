...
`SET(..., EX = retention_seconds)`는 요청 기록에만 만료를 설정한다는 뜻의 슈도코드입니다. 실제 구현에서는 요청 ID의 허용 형식·소유 범위·보관 기간을 서비스 계약으로 정하고, 요청 키는 인증된 주체·작업 종류·요청 ID에서 신뢰된 서비스가 만들고, 두 키가 서로 다른지 확인해야 합니다. Redis Cluster에서는 두 키가 같은 해시 슬롯에 있어야 합니다.
