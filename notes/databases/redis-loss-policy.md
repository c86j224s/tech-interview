...
Redis에서 키가 사라졌다는 관찰은 시간 만료, 용량 압박에 따른 eviction, 애플리케이션 삭제, 복구 과정의 손실을 구분해 주지 않습니다. 먼저 그 키가 재생성 가능한 캐시인지 권위 상태인지 분류하고, 그 다음 제거 정책과 메모리 계측을 연결해야 손실을 허용할 범위와 쓰기 실패 처리를 정할 수 있습니다.
...
진단은 한 시점의 `GET`보다 전후 계측을 비교하는 방식이 안전합니다. 예를 들어 `expired_keys`가 늘지 않았는데 `evicted_keys`가 늘고 쓰기 시 OOM 오류가 났다면 용량 경로의 증거가 강합니다. 반대로 두 카운터가 그대로이고 애플리케이션 감사 로그에 `DEL`이 있으면 eviction으로 단정하지 않습니다. 이 기록은 설계 예측이며 실제 인스턴스 실행 결과와 구분합니다.
