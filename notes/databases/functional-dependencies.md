...
함수 종속은 현재 표본의 모양을 요약하는 문장이 아니라, 허용된 모든 상태에서 같은 결정자 값이 어떤 결과를 하나로 정한다는 업무 규칙입니다. 먼저 유일성의 주체·범위·기간을 정하고, 그 규칙을 폐포와 후보키로 계산한 다음 실제 DB 제약과 동시 쓰기에서 확인하는 순서가 안전합니다.
...
작은 검산은 `A→B, B→C, AC→D`에서 `A+`를 계산한 뒤 A를 제거한 집합의 폐포가 전체 속성을 만들지 못하는지 확인하는 방식으로 시작할 수 있습니다. 이어 두 세션에서 같은 tenant·정규화 이메일을 동시에 삽입해 한 요청만 성공하는지, 이메일 이력에는 같은 주소가 다른 기간에 다시 등장할 수 있는지 확인하면 수학적 후보키와 실제 업무 제약을 혼동했는지 드러납니다.
