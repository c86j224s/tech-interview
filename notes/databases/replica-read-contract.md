...
## 긴 Replica 조회가 Replay와 원본 정리에 비용을 옮길 수 있습니다
...
## 승격하면 성공했던 쓰기가 새 원본에 없을 수 있습니다

비동기 복제에서 원본이 성공 응답한 로그를 새 leader가 확보하지 못했으면 승격 뒤 그 쓰기가 사라질 수 있습니다. 단순히 replay 전이라는 이유만으로 반드시 손실되는 것은 아니며, 수신·내구 로그가 있어 승격 전에 적용 가능한지 실제 복구 프로토콜을 확인합니다.
...
## 저장부터 다시 읽기까지 한 Timeline으로 시험합니다

테스트에서는 저장 직후 replica 읽기·receive와 replay 차이·느린 보고서·원본 우회 포화·승격·old token을 나눕니다. 성공 응답된 쓰기와 응답 불확실 요청을 구분하고 최종 원장·버전·사용자 결과를 확인합니다. 현재 작업에서는 실제 복제·승격을 실행하지 않았습니다. 본문은 관찰·보존 범위의 설계입니다.
