...
Python에서 예상 밖의 상태 공유는 값이 복사됐는지보다 객체가 언제 만들어졌고 어떤 참조 경로가 남았는지에서 생깁니다. 기본 인자·closure·shallow/deep copy를 호출 시점과 객체 정체성의 추적으로 비교합니다.
...
함수 객체의 `__defaults__`에 저장된 리스트가 두 호출 사이 같은 identity인지 `is`로 확인하면 값 누적의 원인이 드러납니다. 호출별 상태가 필요하면 None/sentinel 분기와 명시적으로 전달된 빈 리스트의 의미를 구분하고, caller-owned list를 수정할지 복사할지도 API에 적습니다.
...
loop가 끝난 뒤 late closure를 실행하면 세 함수가 같은 cell의 현재 값 2를 읽고, `lambda i=i`는 각 정의 시점의 값을 보관합니다. `now=clock()`도 mutable 여부와 별개로 정의 시점에 고정되므로 요청별 시각은 함수 본문이나 명시적 clock 인자로 받습니다.
...
dict 자체의 identity는 달라도 `original["items"] is shallow["items"]`는 참일 수 있습니다. 따라서 child에 append하는 변형과 top-level child를 새 리스트로 대입하는 변형을 각각 실행해 어느 경로가 원본에 보이는지 예측합니다.
...
한 번의 `deepcopy`에서는 `a`와 `b`가 같은 원본 child를 가리키는 별칭이 결과에서도 같은 새 child를 가리키고, self link도 결과 자신을 가리킵니다. 항목별로 따로 복사하면 memo 경계가 달라져 별칭 보존 기대가 바뀐다는 점을 테스트에 포함합니다.
...
파일·소켓·lock을 복사해도 OS 자원이나 동기화 의미가 분리됐다고 볼 수 없습니다. 큰 mutable graph를 복사하는 대신 immutable snapshot 또는 필요한 DTO를 만드는 선택을 먼저 비교하고, 복사 중 writer가 바꿀 수 있으면 lock/version 경계를 둡니다.
...
값이 같은지와 identity가 같은지를 함께 검사합니다. 인자 생략·빈 리스트 전달·None·sentinel·재정의·closure 재대입·공유 child·순환 graph·외부 handle을 나누면 “복사됐다”라는 한 문장으로 서로 다른 계약을 덮지 않게 됩니다.
