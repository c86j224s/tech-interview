...
읽기 전용으로 Animal을 돌려주는 자리에 Dog만 돌려주는 구현을 둘 수 있는 경우와, Animal을 쓸 수 있는 가변 자리에 Dog 전용 저장소를 두는 경우는 다릅니다. 후자는 다른 Animal을 쓰면 Dog 계약이 깨지므로 단순 공변으로 취급할 수 없습니다. 사용 checker·Python 문법 버전에 맞게 선언합니다. runtime isinstance로 모든 parameterized Protocol 타입 인자를 검증할 수 있다고 보지 않습니다.
...
`L(D) = [D] + merge(L(B), L(C), [B,C])`로 계산합니다. 각 목록의 head 중 **다른 목록의 tail에 없는 후보**를 골라 결과에 넣고 모든 목록의 앞에서 제거합니다.
...
O는 object입니다. tail에 있다는 것은 그 목록의 head를 먼저 놓아야 한다는 제약입니다. 적합한 head가 없으면 서로 모순된 순서를 요구하므로 클래스 정의가 TypeError로 실패합니다. 예를 들어 한 부모가 X,Y 순서이고 다른 부모가 Y,X 순서를 강제하면 이를 결합한 자식에서 모순이 생길 수 있습니다.
...
B.f에서 super().f를 호출하면 실제 D 객체의 MRO에서 B 뒤의 구현을 찾으므로 C.f로 갈 수 있습니다. 각 참여 메서드가 같은 계약으로 다음 super를 호출해야 전체가 한 번씩 실행됩니다. 중간에 A.f(self)를 직접 호출하면 C를 건너뛰거나 다른 경로에서 A를 중복 호출할 수 있습니다.
