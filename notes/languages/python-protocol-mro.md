...
이 노트는 Python에서 ‘필요한 모양을 갖췄다’는 정적 호환성과 ‘실제로 약속한 행동을 한다’는 런타임 계약을 분리해 설명합니다. 이어 다중 상속에서 C3가 부모 순서를 병합하는 과정을 손으로 추적하고, `super()`가 직접 부모가 아니라 실제 객체 MRO의 다음 구현을 찾는 이유를 확인합니다.
...
구체적으로 `D(B,C)`의 인스턴스에서 `B.f`가 `super().f()`를 실행하면 검색 시작점은 B 다음이고, 그 위치의 C.f가 호출됩니다. 따라서 B의 문법만 보고 A.f가 바로 실행된다고 추측하면 안 되며, 호출 대상은 인스턴스의 `type(obj).__mro__`와 각 클래스의 실제 메서드 존재 여부로 확인해야 합니다.
...
재현 연습은 `D(B,C)`에서 각 `f`가 자신의 이름을 기록한 뒤 `super().f()`를 호출하게 만드는 것입니다. 예상 결과는 `B,C,A` 순서가 한 번씩 기록되는 것이며, B가 A를 직접 호출하도록 바꾸면 C가 빠져 협력 호출 계약이 깨지는 것입니다. 별도로 `runtime_checkable` Protocol에서 메서드 이름만 가진 객체를 검사하면 `isinstance`는 통과할 수 있어도 잘못된 반환 의미·예외·멱등성은 검출하지 못한다는 한계를 확인할 수 있습니다.
