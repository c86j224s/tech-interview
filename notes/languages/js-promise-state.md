...
B가 먼저 실패해 all이 거절돼도 A는 계속 실행될 수 있습니다. race로 timeout을 만들면 사용자 대기는 끝나도 실제 fetch·계산은 남을 수 있습니다. 필요한 경우 AbortSignal 등 API별 취소를 전파하고 실제 종료를 관찰합니다. 이미 서버에서 커밋한 변경은 combinator 거절이나 abort로 되돌아가지 않습니다.
