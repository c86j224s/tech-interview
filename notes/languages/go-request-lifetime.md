...
Go 요청 코드를 읽을 때는 값의 전달 방식, 취소의 전파, 작업의 완료 확인, 자원의 정리를 서로 다른 수명으로 나눕니다. `context.Context`가 호출을 취소할 수 있다는 사실은 함수가 실제로 취소를 관찰한다는 뜻이 아니며, `Wait`가 반환된다는 사실은 외부 효과가 되돌아갔다는 뜻도 아닙니다. 이 네 경계를 따라가면 goroutine 누수와 너무 이른 정리를 구별할 수 있습니다.
...
재현 결과는 goroutine 수만으로 판정하지 않습니다. 첫 오류 시점, 각 자식이 ctx를 관찰하고 반환한 시점, `Wait` 반환 시점, 열린 파일·소켓·lock이 정리된 시점을 순서대로 기록해야 합니다. `Wait`가 끝났는데 외부 작업이 계속되면 그 작업은 그룹의 완료 계약 밖에 있는 것입니다.
