...
JavaScript의 선언과 클로저는 소스 코드가 위에서 아래로 읽힌다는 직관만으로 설명되지 않습니다. 실행 환경이 바인딩을 준비하는 시점, 대입과 초기화가 일어나는 시점, 함수가 나중에 어떤 바인딩을 참조하는지를 분리하면 var의 undefined, let·const의 TDZ, 반복별 클로저, 모듈 순환을 같은 모델로 추적할 수 있습니다.
...
`const read = () => value`를 먼저 만들고 `let value = 3`을 초기화한 뒤 호출하면 함수 생성 시점이 아니라 호출 시점의 바인딩 상태를 읽으므로 3이 나옵니다. 반대로 초기화 전에 호출하면 TDZ ReferenceError입니다. 같은 방식으로 `var` 반복 콜백은 종료 뒤 공유 바인딩 값을 읽고, `let` 반복 콜백은 각 반복 바인딩을 읽는다는 것을 호출 시점까지 따라가야 합니다.

## 안쪽 바인딩 초기화 전 Shadowing 적용
