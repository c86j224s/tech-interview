...
JavaScript의 값 문제는 비교 연산자 하나를 통일하는 것보다 입력을 어떤 도메인 타입으로 확정할지에서 시작합니다. 이 노트는 빈 문자열과 특수값, 객체의 ToPrimitive 부수 효과, Number의 안전 정수 경계, BigInt와 JSON 왕복을 한 흐름으로 추적합니다.

## 빈 문자열·0 동치와 입력 검증

입력 경계에서는 `raw === ""`을 먼저 처리하고 허용 문법을 검사한 뒤 숫자로 변환하는 순서를 고정합니다. 예를 들어 금액 입력에서 빈 문자열을 0으로 해석하면 “미입력”과 “0원”이 합쳐지므로, `===` 사용만으로는 업무 의미의 오류를 해결하지 못합니다.
...
선택 기준은 값의 의미입니다. 계산 결과가 NaN인지 검사할 때는 `Object.is` 또는 Number.isNaN 같은 목적별 API를 쓰고, Set·Map에서 값의 중복을 판정할 때는 SameValueZero를 전제로 합니다. `+0`과 `-0`을 구분해야 하는 수치 알고리즘과 일반 키 집합은 서로 다른 계약입니다.
...
제시된 trace를 한 단계씩 읽으면 `String(value)`는 string hint, unary plus는 number hint, `value + 1`은 default hint를 요청합니다. 따라서 로그·비교·덧셈처럼 보이는 코드가 외부 객체의 상태를 바꾸거나 예외를 던질 수 있으며, 도메인 경계에서는 원시 문자열을 검증한 뒤 변환해 hook 실행을 최소화합니다.
...
식별자 `9007199254740993`을 JSON.parse 후 BigInt로 바꾸는 실습의 예측 결과는 이미 `9007199254740992`로 손실된 값입니다. 정확성이 필요하면 JSON 입력 단계에서 문자열로 받고, 산술이 필요할 때만 문자열에서 BigInt를 만들며, Number와의 혼합 연산은 명시적으로 분리합니다.
...
왕복 테스트의 판정값은 단순히 요청이 성공했는지가 아니라 원래 문자열과 최종 문자열이 같은지입니다. 선행 0, 큰 정수, 공백, 지수 표기를 각각 넣고 브라우저·JSON·서버·DB·로그 경계에서 타입이 Number로 바뀌는 지점을 찾으면 “비교 통일”보다 실제 손실 지점을 해결할 수 있습니다.
