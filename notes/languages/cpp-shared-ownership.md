...
C++에서 shared_ptr를 사용할 때는 세 가지를 먼저 분리해야 합니다. 제어 블록이 객체 수명을 세는 일, shared_ptr 변수 자체를 교체하는 일, 그리고 객체 내부 상태를 여러 스레드가 변경하는 일은 서로 다른 동기화 문제입니다. 여기에 weak_ptr는 수명을 연장하지 않고, generation은 객체가 살아 있는지와 별개로 결과가 현재 요청에 속하는지를 판정합니다.
...
실무 선택은 공유 소유가 정말 필요한지부터 시작합니다. 소유자가 하나면 unique_ptr, 읽기 전용 설정 교체면 완성된 불변 snapshot, 여러 비동기 작업이 수명을 함께 잡아야 하면 shared_ptr를 검토하되 내부 상태에는 별도 잠금을 둡니다. 세대 7 callback이 세대 8 이후 적용됐다면 객체가 살아 있었는지는 맞아도 generation 경계가 빠진 것입니다.
...
재현할 때는 객체 주소의 생존만 보지 말고 소멸자 thread와 상태 세대를 함께 기록합니다. weak lock 실패는 정상적인 관찰 실패일 수 있지만, shared_ptr를 루트 변수에서 동시에 교체하며 필드가 섞이면 루트 동기화 문제입니다. ASan·TSan 통과만으로 이런 논리적 순서가 보장됐다고 결론내리지 않고, 각 경쟁 시나리오의 최종 상태를 검사합니다.
