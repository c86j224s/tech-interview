...
call·apply는 해당 한 번의 호출 대상을 지정합니다. bind는 원본을 수정하는 대신 지정한 this와 앞 인자를 기억하는 새 함수를 만듭니다. 일반 호출과 원본이 생성 가능한 경우의 new 호출은 다른 규칙이므로 bind가 모든 호출에서 this를 영구 강제한다고 일반화하지 않습니다.

## 화살표는 생성 문맥의 this를 사용합니다
