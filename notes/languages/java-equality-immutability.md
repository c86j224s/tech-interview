...
HashSet에 값이 "a"인 키 객체를 넣고 그 키의 비교 필드를 "b"로 바꾸었다고 합시다. 저장 당시 해시와 나중 조회 해시가 달라져 같은 객체가 내부에 남아도 contains·remove가 찾지 못할 수 있습니다. 특정 실패 출력이 모든 Set의 보장은 아니며, 저장 중 동등성에 영향을 주는 값을 바꾸면 컬렉션 계약을 신뢰할 수 없다는 것이 핵심입니다.
...
실행문은 main 등에 넣습니다. final 변수에 새 리스트를 대입할 수 없지만 가변 리스트의 add는 가능합니다. unmodifiableList는 그 view로 수정하는 것을 막아도 원본 변경을 반영할 수 있습니다. List.copyOf는 이후 원본의 구조 변경을 반영하지 않는 수정 불가 목록을 주지만 항상 새로운 객체 정체성을 보장하지는 않습니다. null 원소를 거절하는 조건도 있습니다.
...
배열·DTO·중첩 컬렉션 원소가 가변이면 필요한 깊이의 방어 복사나 불변 값 변환이 필요합니다. 생성자 입력만 복사하고 getter에서 내부 배열을 반환하면 다시 별칭이 열립니다. record도 참조 구성요소의 깊은 불변을 자동 제공하지 않으며 배열의 업무상 내용 동등성까지 자동 정의하지 않습니다.

## Final 필드 초기화 보장과 이후 공개를 나눕니다

Java 메모리 모델은 올바르게 생성된 객체의 final 필드에 특별한 초기화 안전성을 제공합니다. 이는 생성 완료 전 this가 다른 스레드로 탈출하지 않는 전제와 관련됩니다. 생성자에서 listener 등록·thread 시작·전역 저장으로 덜 초기화된 객체를 노출하면 기대를 깨뜨릴 수 있습니다.
