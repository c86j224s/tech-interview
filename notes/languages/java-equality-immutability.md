...
Java에서 값의 동등성, 해시 기반 컬렉션의 키 안정성, 목록의 구조 불변성, 객체 그래프의 깊은 불변성은 서로 다른 계약입니다. 먼저 어떤 필드가 값의 identity인지 정하고, 그 필드가 컬렉션에 들어간 뒤 바뀌지 않으며 객체가 완전히 초기화된 상태로 공개되는지 순서대로 확인합니다.

## 가변 키와 HashSet 후보 위치 불일치
...
같은 hashCode가 같은 객체나 같은 값을 뜻하지는 않습니다.

상태 trace는 `key.hashCode()` 계산으로 후보 bucket을 고른 뒤 `equals`로 확인하는 흐름입니다. 삽입 후 비교 필드를 바꾸면 조회가 새 hash 후보로 가서 실패할 수 있으므로, 변경 전 remove→변경→add를 같은 보호 경계에서 수행해야 합니다. Point·ColoredPoint·세 번째 객체를 양방향으로 비교하면 대칭성과 추이성의 반례가 드러납니다.

검증 실습에서는 원본 list에 원소를 추가한 뒤 view와 `copyOf`를 각각 관찰하고, 원소 DTO의 내부 필드도 바꿉니다. 예상 결과는 view가 구조 변경을 따라가고 snapshot은 따라가지 않지만, 두 목록이 같은 가변 원소를 참조하면 DTO 필드 변경은 양쪽에서 보이는 것입니다. 이 결과를 막으려면 목록 복사만이 아니라 원소 변환 또는 방어적 복사가 필요합니다. 충돌은 정상 가능성이며 후보를 찾은 뒤 equals로 구분합니다. 모든 객체가 같은 해시를 내도 방향 계약은 만족할 수 있지만 성능이 나빠집니다. Object의 기본 equals는 참조 동일성이므로 내용 기반 equals를 정의하면 같은 기준의 hashCode도 함께 정의합니다.
...
## Final 필드 초기화 보장과 이후 가변 상태의 가시성
