...
## 함수가 끝났다는 것과 Broker가 확인했다는 것은 다릅니다
...
## Visibility 기한보다 작업이 길면 실행이 겹칠 수 있습니다
...
키만 먼저 저장하고 변경 전에 죽으면 재시도가 키 때문에 건너뛸 수 있습니다. 변경을 먼저 하고 키를 나중에 저장하면 중복 반영됩니다. 한 DB 거래로 묶거나 명시적 상태 머신과 실제 효과 조회를 통해 복구합니다. 외부 API는 그 API의 멱등 키·결과 조회를 사용하고 로컬 started 레코드만 보고 무조건 재실행하지 않습니다.
...
일시 오류·영구 입력 오류·권한 거절·불확실 결과를 구분합니다. 최대 시도·전체 deadline·backoff·jitter·dead-letter 또는 수동 보류를 둡니다. worker lost를 계속 requeue하는 poison task가 다른 작업을 막지 않게 합니다. 재시도 횟수와 논리 효과 수는 별도 지표입니다.
