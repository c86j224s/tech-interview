...
GIL이 활성인 일반 CPython에서 한 인터프리터의 Python 바이트코드는 한 시점에 한 스레드가 실행합니다. 순수 Python CPU 루프를 여러 스레드로 나눠도 여러 코어의 동시 바이트코드 실행을 얻지 못해 이득이 제한됩니다. 반면 파일·네트워크 I/O 대기 중 GIL을 놓는 경로에서는 다른 스레드가 진행하며 대기를 겹칠 수 있습니다.
...
fork는 부모의 주소 공간을 복사한 상태에서 시작하지만 멀티스레드 부모의 모든 스레드를 자식에서 그대로 실행하지 않습니다. 사라진 스레드가 보유했던 lock·native 라이브러리 상태가 안전하게 재구성된다고 가정할 수 없습니다. forkserver도 별도 서버에서 자식을 만드는 운영 계약을 가지므로 플랫폼·Python 버전의 지원·기본값을 확인하고 필요한 시작 문맥을 명시합니다. 라이브러리가 전역 시작 방식을 강제로 바꿔 다른 코드와 충돌하지 않게 합니다.
...
큰 읽기 전용 배열을 한 번 공유하고 작은 offset·shape·dtype 정보를 넘기는 방식은 반복 복사를 줄일 수 있습니다. 그러나 범위 검증·dtype·읽기 종료·동시 쓰기·close·unlink의 소유를 명시해야 합니다. 각 프로세스의 매핑 닫기와 공유 객체 이름 제거는 다른 책임이며 플랫폼·resource tracker 동작도 확인합니다.
