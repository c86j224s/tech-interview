...
전통적인 GIL 활성 CPython에서 참조 수가 0이 되면 보통 즉시 정리가 시작될 수 있습니다. 그러나 A와 B가 서로 가리키는 도달 불가능한 순환은 내부 참조 수가 남아 별도 cyclic GC가 추적 가능한 객체 그래프를 검사해야 합니다. 구현·버전·free-threaded 변화까지 포함한 정리 시각을 Python 언어 전체의 즉시 소멸 보장으로 일반화하지 않습니다.

## 객체 회수와 RSS·외부 자원은 다른 층입니다
...
observer가 대상의 수명을 연장할 필요가 없으면 weakref를 사용할 수 있습니다. 사용 시 `obj = ref()`를 한 번 호출해 강한 지역 참조를 얻고 obj is None이면 사라진 것으로 처리합니다. 존재 확인 후 ref를 다시 호출하는 사이 소멸이 생길 수 있으므로 결과를 지역 변수로 유지합니다.
...
## With는 Exit가 무엇을 하는지에 따라 결과가 달라집니다

context manager의 __enter__가 정상 진입한 뒤 블록을 벗어나면 __exit__가 예외 정보와 함께 호출됩니다. __exit__가 truthy 값을 반환하면 블록 예외를 억제할 수 있습니다. 자원을 정리한다는 것과 오류를 성공으로 바꾼다는 것은 다른 결정입니다.
...
release_resource는 실제 자원 함수입니다. 그것이 예외를 던지면 cleanup 오류가 전면에 나타날 수 있으므로 원래 오류의 예외 연결과 보조 오류 기록을 보존합니다. DB manager가 정상 종료에 commit하는지, 연결 반환만 하는지 제품별로 확인합니다. 블록 안에서 오류를 catch해 삼키면 manager에는 정상 종료로 보일 수 있어 의도치 않은 commit을 만들 수 있습니다.

## 진입 중 실패는 이미 얻은 자원만 정리합니다
...
## 예외가 어디서 사라지는지와 자원이 언제 닫히는지 봅니다
