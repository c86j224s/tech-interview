...
Java에서 객체 메모리의 생존은 GC root에서의 도달성으로 결정되지만, 파일·소켓·스레드·클래스 로더 같은 외부 자원은 별도의 종료 계약을 가집니다. 따라서 “객체가 언젠가 수집된다”와 “자원이 지금 반납된다”를 분리해 소유 경로, close 시점, 재배포 수명을 각각 추적해야 합니다.
...
A와 B가 서로 가리켜도 root 경로가 끊기면 수집 후보가 되지만, 살아 있는 thread가 A를 지역 변수로 들고 있으면 순환 전체가 살아 있습니다. 진단에서는 참조 개수보다 root path를 먼저 보고, 수집 후보라는 결론을 FD나 연결이 닫혔다는 결론으로 확장하지 않습니다.
...
heap dump에서 retained size가 큰 노드를 찾은 뒤 root path와 기능 종료 후 필요한지 여부를 함께 확인합니다. static cache가 원인처럼 보여도 실제로는 ThreadLocal이나 context class loader가 같은 객체를 붙잡을 수 있습니다. 수정 방향은 보유자를 끊는 것인지, 정상 cache의 용량·만료를 제한하는 것인지로 나뉩니다.

## 파일·연결 자원과 Try-with-resources 범위
...
본문 `consume`이 예외를 던지고 `close`도 실패하는 경우 호출자는 본문 예외를 주 예외로 받고 close 예외를 suppressed에서 찾아야 합니다. 작은 테스트의 예측 결과는 close가 한 번 호출되고 suppressed 목록에 정리 실패가 남는 것입니다. 다만 close가 외부 저장의 내구 commit을 보장하는지는 자원 API 계약을 별도로 확인해야 합니다.

## Cleaner와 명시적 정리의 역할 분담
...
Cleaner를 넣었을 때도 정상 경로의 `close` 호출 횟수를 카운트하고, close 누락 시에만 보조 정리가 일어나는지 확인합니다. 정리 action이 wrapper를 강하게 캡처하면 Cleaner 자체가 wrapper를 살릴 수 있으므로, 보조 상태의 참조 그래프를 별도로 검토해야 합니다.

## 재배포 시 옛 Class Loader 접근 경로 차단
...
재배포를 반복할 때 살아 있는 loader 수와 metaspace가 함께 증가하면 listener·executor·ThreadLocal·context class loader를 root별로 조사합니다. `System.gc()` 한 번 뒤 수치가 내려갔다는 결과는 종료 순서와 장기 추세를 증명하지 않으므로, 반복 load/close 뒤 강한 참조가 남지 않는지와 열린 FD를 별도 측정합니다.
...
검증은 deinit 또는 heap 감소 하나로 끝내지 않습니다. close 전·후의 열린 FD, worker 수, listener 등록 수, loader 수, metaspace를 각각 기록하고 예외 경로에서도 같은 종료 계약이 실행되는지 확인합니다. 실제 JDK·앱 서버의 loader 정책은 대상 환경에서 확인해야 하며, 실행하지 않은 실험을 완료한 것처럼 쓰지 않습니다.
