...
취소는 작업을 지우는 명령이 아니라 중단을 요청하고 종료를 관찰하는 프로토콜입니다. 호출자에게 응답을 끝내는 시점, 작업이 취소 신호를 받은 시점, 버퍼·연결·허가를 더 이상 사용하지 않는 시점을 따로 기록해야 자원 상한과 외부 효과를 올바르게 말할 수 있습니다.

## 응답 대기 종료·취소 요청·실제 작업 종료
...
실습에서는 허가 2개와 취소에 늦게 반응하는 작업 3개를 두고, 응답 timeout 직후 허가를 반환하는 구현과 실제 종료 뒤 반환하는 구현의 최대 활성 작업 수를 세어 보십시오. 예상 결과는 전자의 경우 일시적으로 3개가 실행되고, 후자의 경우 2개를 넘지 않는 것입니다. callback 횟수와 열린 버퍼 수는 허가 수와 별도 assertion으로 둡니다.
