...
조건 변수는 “이벤트를 한 번 저장해 전달하는 큐”가 아니라 공유 상태가 바뀌었을 때 조건을 다시 볼 수 있게 하는 대기 장치입니다. 작업 자체를 보존해야 하면 큐가 맡고, 현재 상태가 조건을 만족하는지 보호해야 하면 mutex가 맡습니다. 따라서 생산자·소비자가 서로 다른 mutex를 사용하거나 큐를 잠금 밖에서 읽으면 wait의 정확한 계약이 깨집니다.
...
이 슈도코드를 점검할 때는 먼저 상태 전이를 적습니다. `stopping=false, queue=[]`에서는 대기하고, 작업 삽입 뒤에는 깨어나도 다시 큐를 확인하며, `stopping=true` 이후에는 큐가 비었을 때만 `closed`를 반환합니다. 이 세 상태의 예상 결과가 맞지 않으면 notify 횟수보다 predicate 보호와 종료 순서를 먼저 조사해야 합니다.
