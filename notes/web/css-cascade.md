...
## 부모 선택자가 강해도 자식의 직접 선언을 덮지 않습니다
...
## 먼저 맞는 선언을 찾고 우선순위를 비교합니다

매칭되는 선택자와 활성 media 조건 등을 확인한 뒤 출처·중요도, 레이어, 명시도, scope 근접성, 소스 순서 등의 규칙으로 승자를 정합니다. 애니메이션·트랜지션·inline과 Shadow DOM의 경계도 관련될 수 있습니다. 아래 예는 같은 문서의 작성자 스타일이고 animation·transition·inline·scope 경쟁이 없는 경우에 집중합니다.
...
테스트는 같은 요소에 위 레이어 예를 적용한 computed color, important로 바꾼 결과, where·is와 ID 혼합, margin의 세 키워드를 대조합니다. 테마·동적 클래스·중첩 컴포넌트·브라우저 사용자 스타일도 함께 확인합니다. 코드에 적은 색과 화면의 색이 다르다면 최종 승자와 상속 출처를 도구로 추적하는 것이 출발점입니다.
