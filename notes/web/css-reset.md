...
실제 checkbox input을 유지한 채 appearance를 바꾸는 것과 div를 checkbox처럼 그리는 것은 다릅니다. native 요소에는 포커스·키보드·checked·disabled·폼 제출과 라벨 연결이 있습니다. 시각을 직접 그리더라도 이 의미와 상태가 사용자에게 드러나야 합니다.
...
reset 전후 제목·문단·목록·폼이 있는 같은 페이지를 비교합니다. 키보드만으로 입력·선택·제출·오류 수정이 가능한지, 확대·작은 화면·다크 모드·강제 색상에서 상태가 보이는지 확인합니다. 스크린 리더의 이름·역할·상태와 목록·제목 탐색도 별도입니다.
