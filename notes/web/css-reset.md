...
CSS reset은 브라우저 기본 표현을 지우는 기술이 아니라, 어떤 의미·조작·상태 표현을 작성자가 다시 소유할지 정하는 계약입니다. 시각적 일관성을 얻더라도 포커스, 폼 의미, 목록 구조, 강제 색상에서 잃는 동작이 없는지 같은 사용 흐름으로 확인해야 합니다.

선택 기준은 “픽셀 차이를 없앴는가”가 아니라 “지운 기본값을 제품 스타일이 대체했는가”입니다. 예를 들어 `outline: 0`으로 기본 윤곽선을 없앴다면 focus-visible 대체 표현이 있어야 하고, native checkbox를 시각적으로 숨겼다면 실제 입력을 조작 가능하고 접근성 트리에 남기거나, 동등한 이름·checked·disabled 의미와 시각 상태를 제공해야 합니다.
...
## appearance 제거와 native control 책임
...
재현 연습으로 제목·목록·링크·input·button이 한 화면에 있는 fixture를 reset 전후로 열고, 키보드 Tab 순서와 제출 오류를 기록합니다. 예상 결과는 포커스가 모든 조작 요소에서 식별 가능하고, invalid 메시지가 해당 필드와 연결되며, forced colors와 작은 화면에서도 checked/disabled 구분이 유지되는 것입니다. 결과가 다르면 reset 범위를 줄이거나 컴포넌트가 잃은 상태 스타일을 명시적으로 복원합니다.

## Reset 변경과 실제 사용 흐름 검증
