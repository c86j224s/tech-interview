...
## 가장 가까운 의도한 요소를 찾고 소유 범위를 확인합니다
...
이 예는 목록이 존재하고 `data-list-root`를 가진 Element이며 DOM 준비 뒤 실행된다는 전제입니다. contains는 바깥 조상을 제외하지만 중첩 위젯을 구분하지는 못합니다. 가장 가까운 목록 루트를 비교해 하위 목록의 동작을 상위에서 다시 처리하지 않습니다.
...
## Shadow DOM 밖에서는 target이 재지정될 수 있습니다

컴포넌트 내부 버튼을 눌러도 바깥에서는 target이 host로 보일 수 있습니다. `composedPath()`는 전파 경로를 이해하는 데 도움을 주지만 closed shadow의 내부를 무조건 노출하지 않습니다. bubbles=true라고 반드시 Shadow 밖으로 나가는 것도 아닙니다.
...
새 항목 추가·삭제, 내부 span 클릭, 중첩 목록, 비활성 버튼, 자식의 전파 중단, Shadow 경계, 키보드 활성화를 대조합니다. 대상이 삭제된 뒤 비동기 완료가 오면 원래 DOM 참조와 현재 논리 상태가 여전히 유효한지도 확인합니다. 위임의 장점은 코드가 짧다는 것보다 소유 범위가 명확하고 동적 요소의 등록 비용을 줄일 수 있다는 점입니다.
