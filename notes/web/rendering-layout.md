...
이 노트는 브라우저가 스타일·기하·paint·합성 작업을 언제 다시 계산하는지와, 성능 최적화가 의미·접근성·메모리 비용을 어떻게 바꿀 수 있는지 설명합니다. 먼저 읽기와 쓰기의 시간 순서를 추적하고, containment·지연 리소스·사용자 입력을 같은 흐름에서 측정합니다.
...
**강제 동기 레이아웃**은 변경 뒤 기하 정보가 필요한 순간 계산을 앞당기는 것이고, 이런 읽기·쓰기 교차 반복이 레이아웃 스래싱을 만듭니다. 모든 크기 읽기가 항상 비싼 것은 아니며 무효화된 상태와 호출 순서를 확인해야 합니다. 상태 추적은 `write(width) → read(offsetWidth) → write(width) → read(offsetWidth)`처럼 같은 프레임에서 무효화와 기하 읽기가 교차하는지 보는 것입니다. `read all → write all → read all`로 바꾸더라도 변경 전·후 어느 값을 요구하는지 먼저 보존해야 하며, 단순히 rAF로 감싼 것만으로 교차가 사라졌다고 볼 수 없습니다.
...
CLS는 예상하지 못한 레이아웃 이동의 사용자 영향을 측정하는 지표이며 모든 위치 변화가 동일하게 집계되는 것은 아닙니다. 단순히 이동 횟수만 세지 말고 브라우저의 layout-shift 근거와 실제 움직인 영역을 확인합니다. 진단에서는 이동을 일으킨 노드·발생 시각·이미지/폰트/광고 원인·영향 영역을 연결하고, CLS 감소와 입력 지연 감소를 같은 결과로 취급하지 않습니다. 이미지 공간 예약으로 이동이 줄어도 decoding·JavaScript·메모리 때문에 입력이 느려질 수 있습니다.
...
대량 목록 변경과 느린 이미지·폰트를 넣고, 확대·모바일·키보드 탐색에서 같은 사용자 흐름을 끝까지 수행합니다. 그 기록에서 평균 프레임만 보지 말고 입력 지연과 메모리도 함께 측정합니다. 화면이 덜 움직였다는 결과와 더 빨리 반응했다는 결과는 다를 수 있으므로, 두 결과를 같은 사용자 흐름에서 함께 기록하고 비교합니다. 실무 선택은 목록 크기·검색/포커스 요구·접근성 트리·메모리 예산을 먼저 정한 뒤 가상 목록, content-visibility, containment를 비교하는 것입니다. 실제 브라우저와 모바일 폭에서 키보드 포커스, 페이지 검색, 확대, 느린 이미지·폰트 조건을 끝까지 실행해야 합니다.
