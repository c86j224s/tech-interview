...
## 받은 횟수와 포인트를 지급한 횟수는 다릅니다

소비자가 포인트를 DB에 반영한 뒤 ACK 전에 죽으면 broker는 같은 이벤트를 다시 보낼 수 있습니다. 전달을 물리적으로 한 번으로 만들려고 ACK를 먼저 보내면 DB 실패 때 이벤트를 잃습니다. 내구 효과를 먼저 확정하고 재전달을 멱등하게 처리하는 경계를 설계합니다.

inbox는 어떤 논리 이벤트를 처리했는지 기록하는 저장소입니다. 메모리 집합만 사용하면 재시작 뒤 사라지므로 원장과 같은 내구성 요구를 확인합니다.

## 처리 마커와 실제 효과를 같은 거래로 묶습니다
...
## 같은 ID의 다른 Payload는 정정이 아니라 충돌입니다
...
서로 다른 정상 event ID 두 개는 모두 통과하므로 points 읽기→계산→SET으로 갱신하면 lost update가 남습니다. 원자 증가·version·허용 sequence·거래 불변식을 유지합니다. 중복 제거는 이벤트 사이 순서나 금지된 중간 상태를 자동 보호하지 않습니다.

외부 API 효과는 로컬 inbox와 원자적이지 않습니다. 안정된 외부 idempotency key·상태 조회·outbox·보상·수동 대사를 사용하고 로컬 마커만으로 결제 성공을 확정하지 않습니다.

## 동시 중복과 정상 이중 사건을 대조합니다
