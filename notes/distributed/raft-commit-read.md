...
Raft를 읽는 기본 단위는 “같은 로그를 여러 노드에 저장한다”가 아니라 `로그 접수 → 과반 기반 commit → 결정적 state machine apply → client 응답`이라는 서로 다른 경계입니다. 쓰기 응답과 읽기 응답의 최신성은 이 경계 중 어디까지 확인했는지로 정의해야 leader라는 표시만으로 판단하지 않게 됩니다.

## 로그 저장·Commit·Apply·응답의 상태와 확정 범위
...
index 105가 log에 있다는 사실은 해당 노드가 entry를 보유한다는 뜻이고, commit=100은 이후 leader가 그 prefix를 보존해야 한다는 뜻이며, apply=97은 업무 상태가 아직 97까지만 반영됐다는 뜻입니다. 이 네 숫자를 요청 ID와 함께 기록하면 “성공 응답 뒤 재조회에서 값이 안 보임”이 apply 지연인지 응답 계약 오류인지 나눌 수 있습니다.

## 현재 Term Entry와 Commit 전진 조건
...
리더가 이전 term entry의 복제 수만 보고 commit했다고 가정하는 것이 흔한 오해입니다. 현재 term entry의 안전한 과반 조건을 먼저 확보하면 그 앞의 접두부가 함께 확정될 수 있으므로, term·match index·현재 구성·저장 응답을 함께 확인해야 합니다. no-op도 이 규칙을 대신하는 마법의 표식이 아니라 현재 term 기준을 확보하는 프로토콜 동작입니다.
...
같은 command가 두 replica에서 apply될 때 current time을 각각 읽으면 timestamp가 달라져 상태가 갈라집니다. 시각·난수·외부 관측은 command에 넣거나 합의된 결과로 바꾸고, logical request ID와 결과를 같은 state transition에 기록합니다. 재시작 후 snapshot에서 dedup 기록이 빠지면 응답 재전송이 새 효과로 변할 수 있습니다.

## Replica별 외부 결제 호출과 효과 중복
...
세 replica가 같은 결제 command를 apply하는 경우 직접 호출 모델은 최대 세 번의 외부 시도를 만들 수 있습니다. outbox worker 모델에서는 합의된 intent 하나와 effect key 하나를 만들고, worker가 결과 조회와 generation 검사를 수행합니다. 그러나 provider가 멱등성을 보장하지 않는다면 Raft의 합의 범위와 외부 효과의 확정 범위를 분리해 미확정으로 남겨야 합니다.

## 고립된 옛 Leader의 로컬 읽기와 최신성
...
A가 stale 값 1을 가진 채 고립되고 B·C가 값 2를 commit한 trace에서, A의 local leader flag는 최신 권위를 증명하지 못합니다. ReadIndex의 quorum 확인 뒤에도 applied index가 read index보다 작으면 적용을 기다려야 합니다. 따라서 관찰 로그에는 quorum 확인 시각과 local apply 대기 시간을 별도 필드로 둡니다.

## Follower의 안전 위치와 로컬 Apply
...
세 중단 지점의 기대 결과는 서로 다릅니다. commit 전에는 새 값이 확정되지 않을 수 있고, commit 후 apply 전에는 재시작 뒤 값이 살아야 하며, apply 후 응답 전에는 재시도해도 dedup 결과를 재사용해야 합니다. 실제 cluster를 실행하지 않았다면 이 기대를 시뮬레이션 결과처럼 쓰지 않고 설계 검증 항목으로만 남깁니다.
