...
Raft 선거는 후보를 하나 고르는 절차만이 아니라, 논리 세대와 내구 투표, 로그 최신성, 과반 규칙을 결합해 옛 권위가 다시 쓰지 못하게 하는 과정입니다. 이 노트는 메시지가 늦거나 노드가 재시작하는 경우에도 어떤 순서가 안전성을 지키는지 따라갑니다.

## Term의 논리 세대와 벽시계 시간의 구분
...
## 투표 응답 전 currentTerm·votedFor 내구 저장
...
## 마지막 Log Term과 index의 최신성 비교 순서
선거 요청의 추적은 `term 확인 → 후보 로그 최신성 비교 → 내구 vote 기록 → 응답` 순서입니다. 중간 저장 전에 응답하거나 lastIndex만 비교하면 재시작 뒤 이중 투표 또는 오래된 로그를 가진 후보의 잘못된 승인을 허용할 수 있으므로 각 경계의 실패 응답을 분리해 기록합니다.
...
## AppendEntries의 prevLogIndex·prevLogTerm 일치 검사
...
## 늦은 메시지와 내구 저장 실패의 분리 검증
오류를 진단할 때 선거가 실패했다는 사실과 안전성이 깨졌다는 사실을 구분합니다. timeout이 짧아 재선거가 잦은 것은 가용성 문제일 수 있지만, 낮은 term을 수락하거나 한 term에 두 번 투표한 것은 프로토콜 불변식 위반이므로 서로 다른 assertion과 로그를 사용해야 합니다.
