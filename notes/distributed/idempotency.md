...
이 예에서는 다음 계약을 택합니다. 재고 행을 잠그고 모든 최종 거절 조건을 먼저 판정한 뒤에 주문과 재고 변경을 시작합니다. 구현상 판정 뒤 부분 변경이 생길 가능성이 있다면 savepoint를 만든 뒤, `FinalBusinessError`가 발생할 때 그 지점으로 되돌린 다음에만 `FAILED_FINAL`과 오류 응답을 기록합니다.

부분적으로 주문이나 재고를 바꾼 채 최종 거절을 함께 확정해서는 안 됩니다. 재고가 다시 들어오면 같은 키를 새로 성공시키고 싶다면 `FAILED_FINAL`로 고정하지 않고 별도의 재시도 정책을 택해야 합니다.
