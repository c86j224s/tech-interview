...
## 멱등성 키의 요청 범위·fingerprint·처리 상태
...
서버는 사용자 `user-42`, 작업 `create-order`, 키 `checkout-20260914-001`을 한 범위로 묶습니다. JSON의 공백이나 필드 순서는 무시하되, 상품·수량·통화·금액처럼 결과를 바꾸는 값은 포함해 정규화합니다. 인증 헤더, trace ID, 이번 시도의 수신 시각처럼 재전송마다 달라지는 값은 fingerprint에서 제외합니다.

선택 필드에 기본값이 있다면 기본값을 채운 뒤 비교해야 합니다. 그러면 같은 의미의 본문은 같은 fingerprint가 되고, 금액만 바꾼 재사용은 충돌로 판별할 수 있습니다.
...
첫 번째 요청 A가 `idempotency_records`에 `PROCESSING` 행을 삽입하면, 같은 범위의 요청 B는 같은 키를 삽입하려고 할 때 그 고유 제약과 부딪힙니다. 이 `PROCESSING` 행은 아직 A의 DB 트랜잭션(transaction) 안에 있으므로 일반 독자는 보지 못합니다.

A가 확정하면 B는 삽입 충돌을 확인하고, A가 확정 전에 중단되어 되돌려지면 B가 키를 차지할 수 있습니다. 즉 이 짧은 작업에서 `PROCESSING`은 다른 요청에게 이미 확정된 진행 상태를 알리는 기록이 아니라, 현재 트랜잭션 안에서 키를 선점했다는 잠정 상태입니다.

별도 트랜잭션으로 `PROCESSING`을 먼저 확정하는 설계라면 lease와 복구 주체를 추가로 설계해야 하며, 아래 흐름의 보장 범위가 아닙니다.
...
이 예에서는 재고 행을 먼저 잠근 뒤 최종 거절 조건을 모두 판정하고, 통과했을 때만 주문 삽입과 재고 차감을 시작합니다. 판정 뒤에도 부분 변경이 생길 수 있으면 savepoint를 만들고, `FinalBusinessError`가 발생했을 때 그 지점으로 되돌린 뒤에만 `FAILED_FINAL`과 오류 응답을 기록합니다. 따라서 잠금·판정·주문·재고 변경·원장 기록이 어떤 순서로 한 트랜잭션에 들어가는지가 이 계약의 실행 경계입니다.

주문이나 재고를 일부 바꾼 채 최종 거절을 확정하면 안 됩니다. 재고가 다시 들어온 뒤 같은 키로 성공시키려는 정책이라면 `FAILED_FINAL`로 고정하지 말고, 그 키를 언제 어떤 조건에서 재시도할지 별도로 정해야 합니다.
...
이 계약에서는 `lockStockRows`가 관련 재고 행을 잠그므로, 재고를 확인한 뒤 차감하기 전에 다른 주문이 끼어들지 않습니다. 모든 최종 거절을 쓰기 전에 판정하는 구현이라면 savepoint는 방어적으로 남아 있는 것이고, `decreaseStock` 같은 후속 단계에서 예상 밖의 `FinalBusinessError`가 나와도 부분 변경을 남기지 않습니다.

`retryWithSameKey`는 다음 시도가 상한을 넘거나 기다릴 시간까지 전체 기한을 넘으면 재귀 호출하지 않고 지정된 응답을 반환합니다. 따라서 `record == null`, `PROCESSING`, 일시 오류의 세 경로가 모두 반환값과 한도를 가집니다.

이 흐름의 `deadline`을 지키려면 DB 연결을 얻는 시간뿐 아니라 잠금 대기와 각 문장 실행에도 남은 시간을 넘겨야 합니다. 시작 직전에 시계를 한 번 확인해도 이미 실행 중인 DB 호출을 30초 안에 끝내도록 만들 수는 없습니다. 커밋 응답이 사라졌을 때는 실패로 확정하지 말고, 현재 활성 트랜잭션만 되돌린 뒤 같은 키의 원장 결과를 다시 읽습니다. `expires_at`은 정리 작업이 결과를 `EXPIRED`로 바꾸는 기준이고, 그 전까지는 저장된 기존 결과를 재현합니다.
...
멱등성 원장을 보관 기간이 끝났다는 이유만으로 삭제하면, 오래된 클라이언트의 재시도를 새 주문으로 볼 수 있습니다. t0에 주문이 성공한 뒤 24시간 후 원장 행을 지웠고, 그동안 네트워크에 갇힌 같은 키가 도착했다고 해 보겠습니다. 서버는 성공했던 첫 주문을 모른 채 두 번째 주문을 만들 수 있으므로, 보관 기간을 캐시 청소가 아니라 “이 키를 얼마 동안 같은 요청으로 인정할지”라는 API 계약으로 정해야 합니다.
...
실습의 판정 기준은 응답 문구보다 최종 상태입니다. 같은 key·같은 fingerprint의 재전송은 주문 행 수와 저장된 response body를 그대로 유지해야 하고, 같은 key·다른 fingerprint는 주문 행을 늘리지 않은 채 충돌을 반환해야 합니다. 커밋 직후 응답을 버린 뒤 조회하면 `SUCCEEDED` 원장과 주문이 함께 보여야 하며, 이 확인 없이는 timeout을 미실행으로 분류하지 않습니다.
