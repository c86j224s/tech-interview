...
이벤트 시스템에서 “순서가 맞다”는 말은 전달, 소비 시작, DB 반영, 외부 효과 중 어느 시점을 뜻하는지 먼저 밝혀야 합니다. 이 노트는 key별 적용 순서와 gap 복구, schema 의미 보존, DLQ 재생을 하나의 상태 전이로 연결해, 큐가 비워졌다는 사실과 업무가 올바르게 복구됐다는 사실을 구분합니다.
...
같은 계정의 전이가 순서에 의존하면 key를 같은 전달 흐름으로 보내고 소비에서도 시작부터 효과 확정까지 직렬화하거나 저장소가 expectedSequence를 원자 검사해야 합니다. 콜백 시작만 직렬이고 await 뒤 다음 이벤트를 실행하면 전체 처리 원자성이 아닙니다. 상태를 `현재 6, 8 수신`으로 두면 8은 bounded buffer에 보류되고 7이 도착한 뒤 7과 8을 차례로 적용해야 합니다. 7이 retention 밖이라면 8을 조용히 버리는 것이 아니라 snapshot 기준점과 이후 delta를 재설정하고, 그 과정에서 중복 event ID와 tombstone을 다시 검증해야 합니다. 이 trace는 전달 순서가 아니라 적용 가능 상태를 기준으로 합니다.

## Snapshot·Delta별 Gap 처리
...
느린 첫 이벤트·역순·영구 gap·미래 큰 sequence·재시작·rebalance·옛 payload·부분 효과 후 DLQ를 시험합니다. 최종 합계뿐 아니라 금지된 중간 상태와 각 정상 사건의 보존을 검사합니다. 실제 검증은 느린 7 뒤의 8, 영구 gap, 재시작·rebalance, 같은 ID의 다른 payload, 외부 효과 후 DLQ를 각각 주입하는 방식으로 합니다. 예상 결과는 정상 key의 prefix가 보존되고, poison event는 bounded retry 뒤 담당 가능한 DLQ 상태가 되며, redrive 후에도 원래 ID와 effect dedup 기록이 유지되는 것입니다.

현재 작업에서는 실제 메시지 시스템의 재생·DLQ를 실행하지 않았습니다.
