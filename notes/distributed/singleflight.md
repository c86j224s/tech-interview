...
Singleflight는 결과 캐시가 아니라 같은 키의 진행 중 원본 작업을 하나로 합치는 조정 장치입니다. 한 요청이 리더가 되어 조회를 실행하고 다른 요청은 그 실행의 결과를 기다리지만, 요청별 timeout·취소와 공유 작업의 수명은 다를 수 있습니다. 따라서 핵심 상태는 `key → flight 세대 → 원본 작업 → 대기자`의 관계로 추적해야 합니다.

## 동일 키 캐시 miss의 중복 조회 집중
...
## 단일 키 flight의 등록·완료 추적

예를 들어 A가 0ms에 flight를 만들고 B가 10ms에 참가한 뒤 A만 100ms에 취소되면, A의 waiter count만 줄고 공유 조회는 계속되어 B가 180ms에 결과를 받을 수 있습니다. 반대로 마지막 대기자가 떠났을 때 조회를 취소하는 정책이라면 원본 API가 실제 취소됐는지 확인하고, 취소되지 않은 작업의 동시성 허가를 즉시 되돌리지 않아야 합니다. 이 상태 trace가 있어야 “대기자는 취소됐지만 DB 작업은 남음”을 정상 동작과 누수로 구분할 수 있습니다.
...
선택 기준은 중복 비용과 조정 비용의 비교입니다. 한 프로세스의 인기 키에는 로컬 singleflight가 단순하지만, 여러 인스턴스에서 같은 키가 동시에 miss하면 공유 lease나 중복 허용 예산이 필요합니다. 다만 lease 만료 뒤 옛 작업이 계속될 수 있으므로 원본 쓰기에는 버전·멱등성 검사를 별도로 두며, 서로 다른 키가 동시에 miss하는 장애에는 전체 원본 admission이 우선입니다.
