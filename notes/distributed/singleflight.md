...
전체 대기 예산을 먼저 예약한 뒤 생성·참가를 원자적으로 수행합니다. 그렇지 않으면 상한 초과로 반환하면서 실행되지 않는 flight가 맵에 남을 수 있습니다. 객체 생성·등록 중 예외가 나면 예약한 예산을 돌려주고, 참가 해제는 대기자 수와 전체 예산을 정확히 한 번씩 감소시킵니다. `startOnceOrSetFailure`는 실행기 제출 실패도 결과 실패로 확정해 모든 대기자를 깨워야 합니다.

완료 뒤 `flights.remove(key)`만 무조건 호출하면 옛 실행의 늦은 정리가 같은 키의 새 실행까지 지울 수 있습니다. 현재 맵 항목이 **자기 flight 객체 또는 고유 세대**인지 확인합니다. 실행 자체는 맵 잠금 밖에서 해야 다른 키의 조회까지 막지 않습니다.
