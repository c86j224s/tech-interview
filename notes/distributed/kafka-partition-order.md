...
Kafka의 순서는 topic 전체에 하나의 번호가 붙는 방식이 아니라 각 파티션 로그 안에서 성립합니다. 따라서 같은 업무 키가 어느 파티션으로 계산되는지와 파티션 수가 바뀔 때 그 계산이 어떻게 달라지는지를 먼저 추적해야 하며, 증설·topic 전환은 순서 보장 계약을 다시 세우는 작업으로 다뤄야 합니다.

## 동일 키의 파티션 내부 순서와 전역 순서의 경계
...
## key bytes·현재 파티션 수와 목적지 계산

producer가 파티션을 직접 적지 않으면 `partitioner`가 레코드의 목적지 파티션을 계산합니다. Kafka 4.3 producer 설정 문서의 기본 동작은 key가 있을 때 그 key의 해시를 사용하고, key가 없을 때는 sticky한 파티션에 배치하는 방식입니다. 그러므로 두 producer가 같은 문자열을 보냈다는 사실만으로는 부족합니다. UTF-8 인코딩·정규화·숫자 표현이 다르면 같은 업무 키도 다른 key bytes가 될 수 있고, 사용자 정의 partitioner가 다르면 다른 파티션으로 갈 수 있습니다.
...
파티션 증설은 소비자 수를 늘리는 일과 다릅니다. 예를 들어 증설 전 같은 key의 이벤트가 P1에 남아 있고, 이후 이벤트가 P4로 가면 P4를 읽는 consumer가 새 이벤트를 먼저 계정 상태에 적용할 수 있습니다. 그러면 생산 시각으로는 뒤인 이벤트가 실제 상태 변경에서는 앞서며, 잔액 차감·조건부 상태 전이·환불처럼 교환할 수 없는 연산의 결과가 달라집니다. 반대로 `+10`과 `+20`처럼 우연히 교환 가능한 연산에서 문제가 안 보였다고 모든 연산도 안전하다고 일반화할 수 없습니다.
...
### 새 Topic 전환의 old·new 로그 위치 기록

새 topic으로 옮길 때는 schema, partition 수, retention, consumer group의 시작 위치를 함께 정할 수 있지만, old offset `100`과 new offset `100`은 서로 다른 로그의 위치일 뿐 같은 사건을 가리키지 않습니다. 전환 기록에는 old의 마지막 입력 위치와 실제 외부 효과 완료 위치, new의 첫 입력 위치와 생산 세대를 따로 남기고, 양쪽에서 같은 논리 event ID를 유지합니다.

두 topic에 동시에 발행하는 dual publish가 한쪽만 성공하면 재시도·outbox·대사로 다른 쪽 전달을 복구하고, 중복 소비는 처리 ID로 흡수합니다.
