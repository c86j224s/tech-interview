...
파일 공개는 “새 내용을 파일에 썼다”와 “독자가 완전한 새 버전을 본다”, “전원 장애 뒤에도 그 버전이 남는다”를 서로 다른 보장으로 나누는 문제입니다. 임시 파일에서 완성본을 만든 뒤 검증하고, 원자적인 이름 공개와 내구화 경계를 분리하면 부분 파일·구버전 조합·재시작 후 사라짐을 각각 진단할 수 있습니다.

## 기존 파일 직접 쓰기와 부분 관찰 위험
...
단일 파일은 파일 교체와 부모 디렉터리 내구화가 핵심이지만, 여러 파일 묶음은 각 파일을 순서대로 바꾸는 대신 immutable version과 manifest 포인터를 사용해야 합니다. 독자는 포인터를 한 번 읽은 뒤 그 버전 디렉터리만 사용해야 하며, 그렇지 않으면 포인터가 바뀌는 사이 서로 다른 버전의 파일을 조합할 수 있습니다. 이 선택은 파일 수와 rollback 보존 기간을 함께 결정합니다.

## 독자의 원자 관찰과 전원 장애 내구성의 분리
