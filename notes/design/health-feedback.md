...
지연이 오르면 먼저 원본 포화인지 외부 API 장애·network·GC인지 나눠 봅니다. 예를 들어 현재 in-flight가 100인데 새 limit을 50으로 낮췄다고 해서, 이미 실행 중인 100개 중 50개가 즉시 종료된 것으로 계산하면 안 됩니다. 새 작업 수락만 줄이고 기존 작업은 실제 완료할 때까지 자원을 점유하게 해야 합니다. 따라서 controller가 겨냥하는 limit과 실제 남아 있는 실행 수를 별도 상태로 기록합니다.
