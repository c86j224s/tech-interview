...
## 재고 0만 보면 두 요청의 잘못된 성공을 놓칠 수 있습니다

재고 1에서 A와 B가 모두 1을 읽고 각각 0을 저장하면 최종 값은 비음수지만 두 판매가 성공했을 수 있습니다. 읽기 완료 지점에 barrier를 두고 두 요청이 도착한 뒤 쓰기를 제어합니다. 최종 재고·조건부 UPDATE 영향 행 수·성공 요청 수·판매 원장·version을 함께 확인해야 합니다.
...
## 가상 Clock과 Timer 실행은 별도 제어입니다

clock.now만 바꿔도 timer callback이 자동 실행되는 것은 아닙니다. 시각 조회·timer 등록·scheduler 실행 queue를 주입하고 테스트가 시각을 전진시킨 뒤 어떤 callback부터 실행할지 정합니다. deadline 직전 완료→timeout, timeout→늦은 완료, 같은 시각의 두 순서를 각각 검사합니다.
...
## 내부 변수보다 이미 존재하는 효과 경계에 주입합니다

network·저장소·clock·scheduler adapter에서 commit 전 실패·commit 후 응답 유실·소유권 전환을 만듭니다. 모든 private 변수에 test flag를 달면 구현에 과결합합니다. production 기본 경로는 정상 동작을 하고 외부 요청이 실패 주입 옵션을 켤 수 없게 합니다.
...
fake는 정책 분기를 빠르게 검사하고 real DB는 고유 제약·lock·격리·연결 실패를 검사합니다. fake의 단일 thread map이 실제 DB보다 강한 보장을 가릴 수 있습니다. exists+insert가 경쟁을 강요한다면 port를 원자 생성 계약으로 바꿔야 합니다.

## 종료·Pause·Partition은 다른 실패입니다
