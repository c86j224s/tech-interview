...
권위 계산 지점의 monotonic/저장소 시간 계약을 쓰고 여러 host의 벽시계를 무작정 섞지 않습니다. 동시 요청이 같은 남은 token을 보고 각각 허용되지 않게 합니다. 상태 eviction·restart 때 매번 full bucket을 주면 반복 초기화로 전체 허용량이 늘 수 있습니다.
...
## 큰 요청은 더 많은 Token을 쓰지만 무한 대기시키지 않습니다
