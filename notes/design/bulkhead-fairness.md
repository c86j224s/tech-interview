...
핵심 예약 3·추천 예약 2·공용 5라면 추천이 공용 5를 쓰더라도 핵심 3은 보호할 수 있습니다. 장애 때 새 차용을 막고 기존 차용은 deadline·협력 취소·실제 완료로 줄입니다. 긴 비선점 작업을 핵심 예약까지 빌려줬다면 필요 순간 즉시 회수할 수 없으므로 “최소 즉시 보장”이 아닙니다.
...
낮은 작업이 1초 동안 worker를 점유하면 높은 작업이 도착해도 다음 선택 기회까지 기다릴 수 있습니다. chunking·안전한 취소 지점·별도 실행/connection 예산으로 영향을 줄입니다. 별도 worker도 낮은 작업이 가진 shared lock·DB connection을 기다릴 수 있어 우선순위 역전과 일반 queue starvation을 구분합니다.

고우선순위가 계속 들어오면 낮은 작업은 영원히 밀릴 수 있습니다. 클래스별 최소 슬롯·가중 round-robin/deficit·aging을 사용하되 만료된 작업은 먼저 제거합니다. aging 상한 때문에 절대 높은 작업을 넘지 못한다면 그것만으로 진행을 보장하지 못합니다.

## 요청 건수 비율과 자원 공정성은 다릅니다

1ms 작업 9개와 1초 작업 1개를 번갈아 실행하면 건수는 9:1이어도 시간이 거의 긴 작업에 갑니다. CPU·bytes·connection 보유 같은 비용을 반영하고 추정 오차를 다음 budget에 보정하거나 작업을 작은 단위로 나눕니다. 개별 실행이 무한하면 최소 슬롯만으로 대기 상한을 보장할 수 없습니다.
