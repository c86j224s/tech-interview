...
프로토콜 통합 테스트의 핵심은 패킷 한 개의 모양과 업무 상태의 변화를 같은 것으로 보지 않는 데 있습니다. 연결을 실제로 진행하면서 원장 효과, 재접속, 이벤트 인과, 허용 가능한 도착 순서를 각각 검증합니다.
...
상태 추적을 `Disconnected → Authenticated(token, sessionVersion) → ActionAccepted → RewardRecorded`로 두고, 각 전이에 응답과 원장 event ID를 붙입니다. 보상 commit 뒤 응답을 버린 재접속은 동일 논리 요청을 다시 제출하는 입력이며, 기대 결과는 응답 두 개가 아니라 원장 효과 한 건입니다.
...
golden bytes는 wire 계약을 고정하고 상태 기반 client는 현재 응답을 다음 요청의 입력으로 삼습니다. 두 방식의 serializer가 같다면 독립 golden decoder나 schema 검사가 필요하다는 판단이 나옵니다. replay fixture에는 만료·비밀·실제 결제 대상이 없어야 하며, 이 내용은 보안상 fixture 경계이지 운영 데이터 복제가 아닙니다.
...
`[L,A,W]`와 `[L,W,A]`를 통과시키고 `[A,L,W]`, `[L,A,A,W]`, `[L]`을 각각 실패시키는 작은 oracle을 둡니다. event ID는 집합이 아니라 다중집합으로 세어 중복 횟수를 보존하고, sequence는 같은 key의 순서를 확인하는 별도 필드로 남깁니다.
...
기대 조건은 “500ms 뒤 성공”이 아니라 “event ID R가 원장에 있고 sessionVersion이 7”처럼 관찰 가능한 상태로 씁니다. 조건이 끝나지 않으면 마지막 응답, 현재 상태, 최초 계약 위반, 남은 기한을 출력하고 timeout으로 무한 대기를 끊습니다. sleep을 줄이는 것만으로 flaky test를 고치지 않습니다.
...
매 commit 회귀는 작은 seed와 고정 payload로 인과·중복·재접속을 빠르게 검증하고, 장기 부하는 key 편중과 세션 수명을 닮게 만듭니다. 회귀가 통과하고 부하가 안정적이어도 낮은 확률의 업무 중복을 자동으로 증명하지는 않으므로 원장 assertion을 양쪽에 유지합니다.
