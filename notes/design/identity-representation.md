...
식별 계약은 최소한 표시값, 비교키, 내부 식별자를 세 층으로 나눕니다. 표시값은 사용자가 입력·확인하는 원문이고, 비교키는 충돌 규칙을 적용한 값이며, 내부 ID는 이름이 바뀌어도 자원을 계속 가리키는 안정된 키입니다. 한 층의 변환을 다른 층의 보안이나 권한으로 오해하지 않는 것이 출발점입니다.
...
선택 기준도 분리합니다. 외부 노출 ID에는 추측 저항성과 시각 정보 노출을 검토하고, 정렬이 필요하면 생성 시각을 별도 열로 저장할지 시간 정렬형 ID를 쓸지 비교하며, 멱등성에는 ID 생성기보다 업무 요청 키를 사용합니다. UUID·ULID를 택했다고 유일성 제약과 인가 검사를 생략하지 않습니다.
...
예상 결과는 NFC+casefold 규칙에서 서로 다른 표현의 이름 두 개가 같은 비교키로 충돌하고, 동시에 삽입하면 DB unique 경계에서 정확히 하나만 성공하는 것입니다. 반면 같은 이름으로 의도적으로 두 예약을 만들 수 있는 업무라면 이름 unique만으로 합치지 않고 예약별 멱등키를 별도로 판단해야 합니다.
