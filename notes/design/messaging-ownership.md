...
메시징 선택은 제품 이름보다 “누가 같은 사건을 받아야 하는가”, “얼마나 오래 다시 읽어야 하는가”, “효과가 끝났다는 증거를 누가 보관하는가”에서 시작합니다. 하나의 queue를 여러 service가 공유하는지, 독립 consumer group이 같은 event를 재생하는지에 따라 전달 모델과 책임이 달라집니다. broker의 Ack와 업무 완료도 별도의 경계로 두고 추적해야 합니다.
...
예를 들어 payment 효과가 성공한 뒤 Ack가 유실되면 같은 event가 재전달됩니다. 안정적인 event ID와 완료 ledger가 있으면 두 번째 시도는 기존 결과를 반환하고 외부 효과를 다시 만들지 않아야 합니다. 반대로 효과 전에 Ack가 성공하면 broker에는 완료처럼 보여도 ledger에 미완료가 남으므로, 재시작 회수 경로가 없다는 것이 실제 장애 원인입니다.
...
선택 시에는 먼저 방송인지 작업 분배인지, 보관 중 재생이 필요한지, 순서가 partition·group·message 단위 중 어디까지인지 결정합니다. 그 다음 broker 기본 보장을 업무 ledger·outbox·멱등키로 보완할 비용까지 포함합니다. 운영 중 backlog가 줄지 않으면 receive 수만 보지 말고 visibility 만료로 인한 동시 실행, DLQ 이동, 효과 lag를 함께 확인합니다.
