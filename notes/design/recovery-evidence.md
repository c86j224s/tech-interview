...
이 노트는 백업 파일의 존재를 복구 가능성의 증거로 오해하지 않도록, 복구 지점·복구 시간·서비스 간 사실의 일치·외부 효과의 격리를 한 흐름으로 묶습니다. 먼저 RPO와 RTO의 시작·종료 경계를 고정하고, 각 서비스의 snapshot과 로그 위치를 안정된 업무 ID로 대사한 다음, 복원 환경을 안전하게 검증한 뒤에만 traffic을 허용하는 순서를 설명합니다.
...
## 서비스별 동일 timestamp와 원자 Cut의 보장 한계
...
## Restore 환경의 옛 Mail·Outbox 외부 전송 차단
...
재현 연습에서는 합성 주문 하나를 주문 snapshot에는 존재하지만 결제 snapshot에는 없는 상태, 그리고 외부 결제 원장에는 성공으로 남은 상태로 구성합니다. 예상 결과는 두 시스템의 동일한 벽시각을 정답으로 채택하지 않고, 주문 ID·event ID·외부 원장 조회를 통해 미확정 상태로 분리하는 것입니다. 복구 환경에서 scheduler를 켜기 전 외부 endpoint 차단이 유지되는지도 확인해야 하며, 이 연습은 실제 key 복구나 운영 backup 무결성을 증명하지 않는다는 한계를 기록합니다.
