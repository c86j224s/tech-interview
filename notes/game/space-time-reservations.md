...
시간·공간 예약은 “갈 수 있는 경로”를 찾는 문제를 “동일한 시각에 같은 자원을 누가 점유하는가”라는 권위 상태와 연결합니다. 계획 후보는 실행 허가가 아니며, 예약 성공도 실제 이동 시점의 충돌·세대·속도 조건을 자동으로 보장하지 않습니다. 따라서 셀·간선·몸체 footprint와 시간 horizon을 같은 좌표계에서 추적하고, 실패하면 안전하게 대기하거나 다시 계획해야 합니다.
...
한 tick의 상태를 `계획 후보 생성 → 결정적 순서 부여 → 전체 footprint 조건 확인 → 원자 예약 → 실행 직전 재검증`으로 기록합니다. A와 B가 동시에 같은 cell을 읽어도 순서가 A-B로 정해지면 A만 예약되고 B는 다른 후보를 찾거나 대기합니다. 이후 A가 despawn되어 ID가 재사용되면 generation 검사가 옛 A의 취소가 새 객체의 예약을 지우지 않게 합니다.

두 NPC가 같은 `(cell,tick)`을 비어 있다고 읽은 뒤 각각 저장하면 둘 다 성공하므로, 조회와 확정 사이를 원자 경계로 묶습니다. 권위 tick owner가 의도를 결정적 순서로 처리하거나 `(cell,tick)` 조건부 생성과 version 검사를 사용해 한 요청만 확정합니다. 여러 cell/edge를 차지하는 footprint는 필요한 전체 조건을 확인한 뒤 한 번에 예약하고, 준비 상태를 쓰더라도 부분 예약은 이동 허가로 공개하지 않습니다.
...
예약 record에는 owner, entity generation(같은 ID가 재사용돼도 이전 객체와 구분하는 세대), plan/request ID, 대상 tick, 만료 시각을 함께 둡니다. 취소·재접속·실행 실패가 나와도 현재 요청의 세대와 일치하는 예약만 해제하고, 새 owner가 만든 예약은 지우지 않습니다. 만료 처리와 신규 예약이 같은 틱에 겹치면 어느 쪽을 먼저 적용할지도 고정합니다.

## 짧은 Horizon의 안전한 끝 상태

Horizon의 길이는 탐색 성능만으로 고르지 않습니다. 현재 속도와 제동 거리, 다음 병목의 점유 여부, 계획 실패 시 머물 수 있는 안전 cell을 함께 계산합니다. 예를 들어 H=3에서 tick 3의 목적 cell이 막혀 있는데도 이동을 확정하면 다음 계획이 실패할 때 교차로를 점유한 채 멈출 수 있으므로, 끝 상태 조건에서 그 후보를 제외하거나 안전 대기 상태를 예약해야 합니다.

미래 H틱까지만 예약하면 메모리와 탐색량은 줄지만 H틱 뒤 병목을 미리 보지 못할 수 있습니다. 그래서 horizon 끝에 도착했을 때 안전하게 멈출 위치, 필요한 제동 거리, 다음 구간을 확보했는지를 조건으로 넣습니다. 실제 이동이 늦어져 예정 slot과 어긋나면 후속 slot을 갱신하고 다른 예약과 다시 대조합니다.
...
## Aging과 공간 부족·교착 해소의 한계

한 cell 폭 외길에서 두 몸체가 마주 보고 있고 서로의 교환을 금지하며 뒤로 물러날 공간도 없으면, 우선순위를 바꾸는 것만으로는 이동 해가 생기지 않습니다. 이때는 누가 누구를 기다리는지 나타내는 wait-for cycle, 마지막으로 진행한 시점, 대기 나이, 실패 횟수를 보고 실제로 비켜설 위치와 후퇴 경로를 찾아야 합니다. NPC와 직접 조작 player에게 강제 후퇴를 적용할 권한과 UX는 다를 수 있습니다.
