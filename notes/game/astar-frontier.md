...
A*의 결과는 휴리스틱 공식 하나보다 frontier에서 어떤 후보를 유지하고 언제 닫힌 정점을 다시 여는지에 달려 있습니다. 허용성, 일관성, 종료 조건, 경로 복원 상태를 같은 탐색 trace 안에서 확인해야 최적성 또는 근사 bound를 과장하지 않을 수 있습니다.

선택 trace에서 S를 확장한 뒤 A의 `f=3`, B의 `f=5`라면 A가 먼저 pop되어 G의 비용 6을 제안합니다. 이후 B가 A를 `g=2`로 개선하면 A를 다시 frontier에 넣고 G를 5로 고쳐야 합니다. 이때 확인할 상태는 bestG, parent, open/closed, heap의 stale entry이며, 목표를 발견한 시점과 목표를 pop한 시점을 혼동하지 않습니다. 연습에서는 lazy deletion entry를 하나 남긴 채 pop해 stale 검사가 그 entry를 건너뛰고, 재오픈 entry가 비용 5를 반환하는지 예측한 뒤 실제 trace와 대조합니다.
