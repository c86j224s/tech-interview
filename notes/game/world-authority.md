...
공간 handoff는 상태 복사 문제가 아니라 권위 있는 쓰기 주체를 한 세대에서 다음 세대로 바꾸는 문제입니다. snapshot을 준비했다는 응답과 실제로 새 owner·epoch를 내구 커밋한 순간을 분리하고, 입력·라우팅·전투 효과가 그 전환점을 기준으로 판단하도록 해야 양쪽 서버의 동시 처리를 막을 수 있습니다.
...
전환 구간에 들어오면 입력을 잠시 보류하는 방식이 가장 단순합니다. 무중단으로 입력을 계속 받으려면 크기가 제한된 bounded buffer(임시 입력 저장 공간)에 handoff ID와 sequence(입력 순서 번호)를 함께 기록합니다. 최종 상태에 기록된 lastAppliedInput(마지막 적용 입력)까지의 연속된 적용 구간(prefix) 뒤에 있는 입력만 처리하고, 각 입력은 한 번만 처리합니다. 예를 들어 A가 `owner=A,epoch=4,lastApplied=120`을 기록한 뒤 B가 snapshot과 입력 121까지 준비했지만 ACK가 유실됐다고 합시다. 재시도는 새 handoff를 만들지 않고 handoff ID와 epoch 5 상태를 조회해 121의 적용 여부를 확인합니다. cutover commit이 없으면 A가 계속 유효하고, commit이 있으면 A의 epoch 4 쓰기는 거절되며 B의 epoch 5 prefix 뒤에서만 재개해야 합니다.

ACK가 유실돼도 성공했다고 추측하지 말고 같은 handoff 상태를 조회해 마지막 적용 지점 뒤부터 재개합니다.
...
준비/ACK 전후 crash·늦은 delta·옛 router·경계 전투·접속 종료·보상 event·옛 epoch 쓰기를 시험합니다. 재현 연습에서는 준비 ACK 전, drain 중, cutover commit 직후, 옛 router cache 사용 시점을 각각 중단합니다. 매 경우 저장소에 유효 writer가 하나만 남고, 입력 sequence는 lastApplied 뒤에서만 재개되며, 동일 attack ID의 피해·보상 effect가 한 번만 확정되는지를 확인합니다. 실제 게임 서버의 tick·rewind·네트워크 지연 수치는 이 설계만으로 검증되지 않습니다.

이 노트는 권위 이전 설계이며 실제 분산 게임 server handoff를 실행한 결과는 아닙니다.
