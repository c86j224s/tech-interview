...
이 노트는 경로 탐색의 성공을 곧바로 이동 허가로 취급하지 않는 방법을 설명합니다. 계획 입력의 세대, 실제 몸체의 footprint, smoothing으로 새로 만든 구간, 목표 변화와 예산을 하나의 수명으로 연결해 늦은 결과와 동적 장애를 진단합니다.
...
맵 version은 환경, 목표 generation은 현재 의도, agent profile version은 크기·능력·이동 규칙을 나타냅니다. 목표가 바뀌면 맵이 그대로여도 이전 성공 결과는 적용할 수 없습니다. 결과에는 request ID와 이 세 범위를 함께 연결합니다. 예를 들어 `request=41, map=8, goal=12, profile=3` 결과가 도착했을 때 현재 목표가 generation 13이면 탐색 자체는 성공이어도 적용하지 않습니다. 적용 직전에는 다음 segment의 sweep와 예약 상태를 다시 확인하고, 실패하면 짧은 안전 구간·정지·재탐색 중 정책을 선택합니다.
...
client 시각 smoothing과 서버 권위 이동을 분리하고 서버와 같은 footprint·dt·충돌 규칙으로 보정합니다. 테스트는 문 변경·연속 목표·반경·좁은 corner·weighted terrain·층 portal·늦은 완료·재탐색 폭주를 포함합니다. 실제 이동 성공·비용·오차·폐기율·CPU·보유 snapshot을 봅니다. 증상별로는 “길이만 줄었는데 비용이 증가”하면 가중치 계산을, “닫힌 문을 통과”하면 footprint·의존 영역을, “완료율 저하와 queue 증가”가 함께 보이면 cooldown보다 global budget과 취소 정리를 먼저 조사합니다. 이 노트는 실행 설계이며 실제 게임 엔진 경로 추종 실험 결과는 아닙니다. 따라서 엔진별 collision sweep·NavMesh portal·dt 계약은 대상 엔진의 공식 문서와 실제 권위 서버 시험으로 확정해야 하며, 이 노트의 수치 예를 성능 보장으로 사용하지 않습니다.
