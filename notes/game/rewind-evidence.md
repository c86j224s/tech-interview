...
Rewind 판정의 기본 모델은 과거 세계를 현재 상태에 덮어쓰는 것이 아니라, 서버가 허용한 시각의 읽기 전용 snapshot을 만들어 판정하는 것입니다. 그 snapshot에는 대상뿐 아니라 공격 경로와 장애물의 시간 기준이 함께 들어가야 하며, 결과 효과는 현재 권위 상태에서 공격 ID로 한 번만 적용합니다.

## 공격자 관측 대상과 현재 서버 대상의 시점 차이
...
발사가 늦게 도착하면 서버는 수신 시각을 기준으로 RTT/jitter 관측값, input sequence와 session, 발사 빈도, 입력의 허용 연령을 함께 보고 과거 판정 시각을 계산합니다. RTT/2는 경로가 비대칭이면 정확한 one-way delay가 아니므로, 그 추정값과 별도로 되돌아갈 수 있는 hard rewind window를 제한합니다.

판정 시각을 추적할 때는 client가 주장한 시각, 서버 수신 시각, 관측된 지연 추정, hard window로 절단된 최종 시각을 각각 로그로 남깁니다. 예를 들어 오래된 timestamp가 들어오면 최종 시각은 허용 범위 안으로 잘리거나 거절되어야 하며, client가 임의로 과거를 선택한 값이 그대로 hitbox 조회 시각이 되어서는 안 됩니다.

 미래 timestamp가 오거나 값이 갑자기 변하거나 같은 상한에 반복해서 닿으면 그 시도를 기록하고 거절하거나 허용 범위 끝으로 절단합니다.
...
재현 연습은 정상 RTT, 큰 jitter, 미래 timestamp, window 끝에 반복해서 닿는 입력, teleport 직전·직후의 발사를 나눠 기록하는 것입니다. 예상 결과는 허용 범위 밖 시각이 거절 또는 명시된 경계값으로 처리되고, teleport 경계를 가로지르는 가짜 보간 hitbox가 만들어지지 않으며, 동일 attack ID의 재전송이 체력 효과를 두 번 만들지 않는 것입니다.
