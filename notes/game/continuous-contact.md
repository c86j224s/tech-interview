...
연속 충돌은 tick 끝의 위치를 비교하는 대신 한 tick 동안 두 형상이 만나는 최초 시각을 찾는 모델입니다. broad phase는 후보를 놓치지 않도록 넓게 고르고, narrow phase는 공통 시간 매개변수의 TOI를 계산하며, 접촉 후 남은 시간과 반복 상한까지 포함해야 결과가 안정적입니다.
...
대상도 움직이면 상대 위치·상대 속도와 양쪽 형상·기준 시각을 사용합니다.

상대 운동을 `p(t)=p0+v·t`로 두면 투사체와 대상이 각각 움직여도 같은 `t∈[0,1]`에서 처음 만나는지를 비교해야 합니다. A가 `t=.2`, B가 `t=.6`에 닿는 예에서는 A의 반응으로 속도나 관통 가능성이 바뀐 뒤 남은 구간 `[.2,1]`을 다시 검사해야 B를 놓치지 않습니다. 계산 결과가 `t≈0`으로 반복되면 최소 진행량을 요구하고, 그래도 상한에 닿으면 안전 정지·소멸을 반환하되 충돌 없음으로 기록하지 않습니다.

재현 실습은 얇은 벽과 고속 점 투사체, 반경 있는 투사체, 움직이는 대상, 시작 내부를 각각 입력으로 두고 끝점 검사와 sweep 결과를 비교하는 것입니다. 예상 결과는 끝점 검사에서 miss인 얇은 벽이 sweep에서는 hit가 되고, projectile+tick만 dedup key로 사용한 구현에서는 같은 tick의 B 피해가 사라지는 것입니다. 현재 투사체와 옛 대상 등 서로 다른 tick을 무심코 섞지 않습니다. 시작부터 내부면 새 진입과 달라서 depenetration·마지막 안전 위치·소멸 같은 정책을 적용합니다.
