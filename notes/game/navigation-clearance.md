...
NavMesh는 지상 2.5D 표면에도 여러 층 연결을 표현할 수 있지만 모든 자유 비행 부피 문제를 같은 graph로 다루는 것은 아닙니다. 지역별 표현을 혼합하면 상위 portal에서 하위 공간 입구/출구·높이·agent profile·version을 검증합니다. 상위 연결 존재가 하위 실행 가능성의 증명은 아닙니다.
...
원형 agent가 병진하는 2D 문제라면 장애물과 반경 r 원판의 Minkowski sum을 금지 중심 영역으로 만들 수 있습니다. 폭 W 통로는 접촉·여유 정책까지 고려해 대략 W>2r 조건이 필요합니다. 격자 근사는 추가 양자화 오차가 있습니다. 높이·경사·선회·비원형 회전은 반경 하나로 모두 표현되지 않습니다.
