...
이 노트에서 말하는 비용은 모델 한 번의 가격표가 아니라 하나의 논리 작업이 검증된 결과로 끝날 때까지 발생한 전체 자원입니다. 먼저 논리 작업과 그 안의 물리 시도·도구 실행·승인을 같은 추적 단위로 묶고, 그다음 비용·지연·안전한 fallback을 비교해야 라우팅이 실제 업무 결과를 개선했는지 판단할 수 있습니다.

## 호출 단가와 성공당 전체 비용의 역전

작은 모델이 같은 `schema` 오류를 열 번 보정하면 강한 모델이 한 번 생성한 경우보다 비싸질 수 있습니다. 한 논리 작업의 비용을 계산할 때는 입력·출력·추론 사용량, `cache write/read`, 검색, `sandbox`, 하위 에이전트, 실패 재시도, 사람 검토 비용을 모두 더합니다. 예를 들어 한 논리 작업이 router 1회, 모델 2회, 검색 1회, sandbox 1회 후 검증 실패로 끝났다면 그 작업은 성공 건수의 분모에 넣지 않고 미완료로 분류합니다. 같은 작업을 수정해 다음 시도에서 검증된 성공이 되더라도 첫 시도의 입력·출력·도구 비용은 성공한 논리 작업의 전체 비용에 합산합니다. 이 구분이 없으면 실패를 많이 만드는 저가 경로가 겉보기 성공당 비용을 낮추는 역전이 생깁니다.
...
모델 교체·cache 만료·provider 장애·도구 timeout·정책 거절·부분 trace·로그 sink 실패를 시험합니다. 과제별 오분류·불필요한 fallback·누적 비용·잘못된 완료를 확인합니다. 실무에서는 동일한 입력 집합을 고정하고 모델 경로만 바꾼 대조군을 둡니다. 각 작업에 대해 `final_verified`, `policy_blocked`, `tool_failed`, `unresolved`를 분리해 비용을 집계하고, cold/warm cache와 provider별 실패를 섞지 않습니다. 이 실험에서 비용은 청구서·도구 사용량·검증 결과의 세 기록이 서로 맞을 때만 비교 가능한 수치가 됩니다.
