...
## 내부에 LLM이 있는지보다 외부 계약이 무엇인지 봅니다
...
2026-09-15 확인한 A2A 명세 페이지는 발행 version 1.0.0을 안내합니다. 접수·실행·완료·실패·취소·거절과 입력/인증 대기 등을 구분합니다. 완료·실패·취소·거절의 terminal task에 새 message를 보내 임의 재개할 수 있다고 가정하지 않습니다. 단순 상호작용은 task 없이 direct message로 끝날 수도 있습니다.

## 입력 대기와 재구독을 같은 작업에 연결합니다

입력 대기 UI에는 task ID·목표·현재 요청 필드·상태를 표시하고 사용자의 답을 해당 outstanding request에만 적용합니다. 다른 task의 질문에 답을 붙이거나 중복 응답을 새 실행으로 만들지 않습니다. 인증·승인이 필요한 경우 모델이 사용자의 동의를 대신 만들지 않습니다.
