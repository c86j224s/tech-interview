...
캐시는 원본보다 빠른 복사본이지만, 복사본의 값이 무엇을 의미하는지와 얼마 동안 허용되는지를 함께 저장해야 합니다. 특히 “없음”, “접근할 수 없음”, “원본이 실패함”은 서로 다른 결과이므로 부재 캐시에 같은 표식을 넣지 않는 것에서 설계를 시작합니다.
...
대표 trace는 부재 조회 A, 생성 commit B, 늦은 부재 refill A의 순서입니다. generation 검사가 없다면 생성 뒤에도 404가 다시 게시되고, 검사가 있다면 A의 쓰기가 거절되거나 최신 version과 조건부 갱신에서 탈락해야 합니다. refresh 실패가 반복되어도 `staleUntil`은 늘어나지 않아야 하며, 만료 이후에는 오래된 값을 계속 주는 대신 원본 대기나 명시적 오류로 전환되어야 합니다.
