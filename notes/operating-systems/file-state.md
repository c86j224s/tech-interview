...
파일 I/O에서 이름, 파일 내용, 열린 파일 상태, 가상 매핑, 저장 장치의 내구성은 서로 다른 객체와 시점입니다. descriptor와 mmap이 무엇을 공유하는지, rename이 무엇만 원자적으로 바꾸는지, 전원 장애 뒤 어떤 약속을 검증할 수 있는지를 분리하면 흔한 “성공했으니 안전하다” 결론을 피할 수 있습니다.
...
새 내용을 임시 파일에 완전히 쓰고 같은 파일시스템에서 `rename`으로 대상 이름을 교체하면 독자가 작성 중인 반쪽 파일을 보는 문제를 줄일 수 있습니다. 여기서 `rename`은 파일 내용과 내구성을 한 번에 확정하는 연산이 아니라, 이름이 어떤 완성 파일을 가리키는지를 바꾸는 단계입니다. 따라서 `write`나 `rename`이 성공해도 전원 장애 뒤 데이터와 디렉터리 항목이 보존됐다고 단정할 수 없고, 아래의 flush·fsync와 공개 시점을 따로 지켜야 합니다.
...
상태를 “임시 파일 write 성공”, “임시 파일 fsync 성공”, “rename 성공”, “부모 디렉터리 fsync 성공”으로 따로 기록하면 장애 지점을 구분할 수 있습니다. 예를 들어 rename 뒤 디렉터리 fsync 전에 전원이 꺼졌다면 새 이름 노출과 재부팅 후 보존은 같은 사실이 아니므로, 복구 검사에서 어느 단계까지 완료됐는지를 근거로 판정해야 합니다.
