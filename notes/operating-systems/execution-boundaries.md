...
실행 경계를 이해할 때는 주소 공간, 실행 주체, 권한 모드, 스케줄 상태를 서로 다른 축으로 놓는 것이 출발점입니다. 프로세스는 주로 주소 공간과 자원 경계를 제공하고, 스레드는 레지스터·스택과 스케줄 단위를 가지며, syscall은 현재 실행 주체가 커널 기능을 요청하는 진입입니다. 그래서 syscall 횟수, context switch, CPU 사용률을 같은 사건으로 세면 원인 진단이 틀어집니다.
...
## Ready·Blocked·Running·Suspended 상태 구분
...
## 시스템 호출의 권한 전환과 스레드 전환의 구분

한 I/O 요청의 상태를 `Running에서 syscall 진입 → 커널에서 즉시 처리하거나 Blocked → I/O 완료로 Ready → 스케줄러 선택 → Running`으로 기록합니다. 데이터가 이미 준비되어 있으면 같은 스레드가 모드만 바꾸고 돌아올 수 있고, 기다림이 생길 때만 다른 runnable 스레드가 선택될 수 있습니다. 따라서 syscall 하나당 switch 하나, 또는 I/O 완료 즉시 Running이라는 설명은 모두 과도한 단순화입니다.

파일·소켓처럼 커널 권한이 필요한 기능을 요청하면 현재 스레드가 정해진 진입점으로 들어가 인자·포인터·권한을 검증받습니다. 데이터가 이미 준비돼 있으면 같은 스레드가 커널 코드를 실행하고 사용자 모드로 돌아오지만, I/O가 끝나지 않으면 그 스레드는 잠들고 스케줄러가 다른 runnable 스레드를 고를 수 있습니다.

따라서 syscall 한 번이 곧 context switch 한 번은 아닙니다. stdio 버퍼링은 여러 작은 쓰기를 한 syscall로 묶을 수 있고, vDSO는 일부 시간 조회를 커널 진입 없이 처리할 수 있지만, 어떤 경로가 가능한지는 시계 종류·CPU·OS 설정에 따라 달라집니다.
...
병목을 선택할 때는 `syscall 시간`, `Blocked 체류`, `Ready 체류`, `실제 CPU 실행`, `IPC 대기`, `cache miss`를 같은 요청 ID로 연결합니다. worker 수를 줄여 context switch가 감소해도 Ready 체류나 p99가 늘면 개선이 아니며, vDSO 경로가 사용됐다는 사실도 전체 요청 latency가 낮아졌다는 증거는 아닙니다. 대상 OS와 CPU에서 실제 카운터 의미를 확인해야 합니다.
