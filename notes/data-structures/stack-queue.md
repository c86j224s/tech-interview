...
스택과 큐는 데이터를 담는 배열 모양보다 “어떤 항목을 다음에 꺼내는가”를 표현하는 추상화입니다. 따라서 자료구조의 인출 순서, 작업의 실제 완료 순서, 외부 효과의 되돌림 가능성을 세 단계로 나누어야 undo·redo와 작업 처리의 차이를 놓치지 않습니다.

## 편집 변경과 접수 작업의 LIFO·FIFO 인출 순서
...
상태를 추적할 때는 접수 시각, dequeue 시각, 시작 시각, 완료 시각을 별도로 기록합니다. A·B를 FIFO로 꺼내도 A가 느리면 B의 완료가 먼저일 수 있으며, 접수 순서가 반드시 외부 시스템의 commit 순서가 되려면 같은 owner에서 직렬화하거나 버전 조건을 추가해야 합니다.
...
`execute`에서 currentState를 먼저 바꾸고 undo 항목을 나중에 저장하면, 두 번째 단계에서 실패할 때 화면은 새 상태인데 undo 스택은 예전 상태로 남을 수 있습니다. 반대로 스택부터 pop한 뒤 `prepareUndo`가 실패해도 같은 불일치가 생깁니다. 따라서 `atomically_publish`는 메모리 단일 소유자·잠금·DB 거래 등 실제 저장 경계 안에서 상태와 이력을 함께 바꾸고, 동시에 다른 편집이 들어오면 기준 버전도 확인해야 합니다.

메일 발송이나 결제처럼 이미 외부 효과가 난 작업은 문자열의 이전 상태로 되돌리는 역연산이 없습니다. 그런 경우 undo는 발송이나 결제를 없애는 것이 아니라 환불·정정이라는 새 보정 작업을 기록하는 의미가 되며, 스택 pop만으로 외부 효과가 사라지지 않습니다. 긴 이력은 snapshot·명령 병합·보관 범위를 함께 정해야 합니다.

## 배열 큐의 head·tail·count와 앞 삭제 비용
...
재현 연습은 용량 3 큐에 A·B·C를 넣고 A를 꺼낸 뒤 D를 넣는 경우와, ABC에서 두 번 undo한 뒤 X를 실행하는 경우를 함께 확인하는 것입니다. 예상 결과는 큐의 논리 순서 B·C·D, 물리 배열과 다른 head·tail 상태, X 실행 뒤 redo 비움입니다. 여기에 worker 완료를 의도적으로 역전시키면 FIFO 인출과 완료 순서가 다르다는 사실도 관찰할 수 있습니다.
