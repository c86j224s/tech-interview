...
해시 테이블의 빠른 조회는 해시 계산만으로 완성되지 않습니다. 후보 슬롯을 찾은 뒤 동등성 비교, 충돌 경로 보존, 삭제 표식, 확장 중 세대 전환이 함께 맞아야 하며, 이 노트는 그 상태 변화를 기준으로 구현 선택과 실패를 진단합니다.
...
## 두 가지 충돌 해결 방식
선택 기준은 평균 복잡도 하나가 아니라 키 입력의 공격 가능성, 삭제 비율, 메모리 지역성, 최악 탐사 지연, 동시 갱신 계약입니다. open addressing을 택했다면 capacity가 가득 차는 경우와 tombstone 누적을 별도 상태로 측정하고, chaining을 택했다면 긴 버킷의 비교 비용과 할당 패턴을 확인해야 합니다.
...
## 확장과 점진 재해시
재해시의 상태를 `old+new`, `new only`, `retired`처럼 명시하면 조회와 쓰기의 우선순위를 설명하기 쉽습니다. 한 키의 갱신이 어느 세대에 기록되는지, 이동 중 삭제가 어떤 버전으로 표시되는지, 독자가 옛 배열을 더 이상 참조하지 않는 시점을 각각 관찰할 수 있어야 합니다.
