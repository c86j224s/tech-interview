...
이 구조의 핵심 상태는 “각 원소가 어느 대표자 아래에 있는가”이며 원래 간선 목록 자체가 아닙니다. 그래서 연결 여부를 빠르게 묻는 문제에는 맞지만, 경로 복원·간선 삭제·연결 성분의 이력 조회가 요구되면 추가 구조나 다른 알고리즘이 필요합니다.
...
## rollback과 find의 숨은 변경

`union(0,2)` 직전에 체크포인트를 저장하고, 병합 후 그 지점으로 되돌리려 한다고 하겠습니다. 병합 뒤 `find(3)`가 3→2→0 경로를 압축해 `parent[3]=0`으로 바꾸면, `parent[2]`와 `size[0]`만 되돌려서는 충분하지 않습니다. union 전의 `parent[3]=2`까지 복원해야 하므로, 경로 압축의 변경을 기록하지 않은 rollback은 연결성 상태를 틀리게 남깁니다.

그래서 단순 rollback 변형은 **경로 압축 없이 크기 기반 병합만 사용**합니다. 성공한 각 union이 바꾼 `parent`, `size`, 성분 수를 스택에 쌓아 두고, 체크포인트로 저장한 스택 길이까지 역순으로 되돌립니다.
...
따라서 일반 Union-Find에는 경로 압축을 켜고, 시간 구간 분할이나 분기 탐색처럼 rollback이 필요한 경로에는 압축을 끕니다. 둘을 섞으려면 압축으로 바뀐 모든 부모 포인터를 history에 기록하는 별도 계약이 있어야 하며, “find는 조회이므로 기록할 필요가 없다”는 구현 선택은 성립하지 않습니다.
...
예상 결과는 `mark=2`를 저장한 뒤 성공한 union 두 개와 `find` 여러 번을 수행하고 rollback했을 때 부모·루트 size·components가 mark 시점과 바이트 수준으로 같아지는 것입니다. 연결성만 같고 부모 배열이 달라도 논리적으로는 통과할 수 있지만, rollback 구현 검증에서는 압축 여부까지 별도 불변식으로 검사합니다.
