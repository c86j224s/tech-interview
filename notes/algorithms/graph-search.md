...
questionIds: [bfs-dfs-shortest-path, multi-source-bfs-initialization]
---

# BFS의 층별 탐색과 최단 거리

BFS를 이해할 때는 큐 구현보다 먼저 거리의 단위를 고정해야 합니다. 모든 간선 비용이 같다는 전제에서 큐의 앞쪽이 거리 층을 대표하고, 처음 발견한 부모가 그 층의 최단 경로를 기록한다는 흐름을 따라가면 구현·반례·검증을 한 모델로 연결할 수 있습니다.

## 최단 거리의 의미와 단위 간선 비용 전제
...
구현 전에 `distance[source]=0`을 “아직 이동하지 않음”으로, 이웃 `v`의 값 `distance[u]+1`을 “한 간선을 더 통과함”으로 정의합니다. 이 정의가 고정되면 가중치 0·1이나 양의 임의 비용 입력은 별도 알고리즘 계약으로 분리해야 하며, 단순히 큐를 우선순위 큐로 바꾸는 것만으로는 현재 슈도코드의 방문 불변식을 보존할 수 없습니다.
...
손으로 검증할 때는 각 행에서 `pop` 직전의 큐와 `enqueue` 직후의 큐를 구분해 적습니다. `D`가 `A`에서 먼저 발견된 뒤 `B`에서 다시 보이는 trace에서 `distance[D]`와 `parent[D]`가 유지되어야 하며, 이 값이 바뀌면 방문 시점을 큐 삽입 때 고정하지 않은 구현일 가능성이 큽니다. 단위 간선 비용에서 목표 경로만 필요하면 최초 발견 시 종료도 가능합니다. 다만 전체 거리표를 반환하는 계약이라면 나머지 탐색까지 끝내거나 부분 결과임을 명시해야 합니다.
...
가장 가까운 응급 시설까지의 거리를 구한다면 시설마다 BFS를 반복하지 않고, 모든 시작점을 거리 0으로 큐에 넣은 뒤 같은 탐색을 수행할 수 있습니다. 중복 시작점은 한 번만 등록하고 부모는 NONE으로 둡니다. 이 초기화는 가상의 공통 출발점에서 비용 0으로 각 시설에 연결한 것과 같은 거리 의미를 가집니다.

```text
for source in unique(valid_sources):
    distance[source] = 0
    parent[source] = NONE
    owner[source] = source
    queue.push_back(source)
...
무방향 경로 A-B-C-D-E에 A와 E를 시작점으로 넣으면 두 시작점이 각각 거리 0으로 큐에 들어가고, 그 뒤의 전파로 거리는 [0,1,2,1,0]이 됩니다. C는 두 원천에서 같은 거리 2로 도착하므로 먼저 큐에 넣은 원천을 `owner[C]`에 남길 수 있습니다. 이때 `parent[C]`를 따라가면 임의의 고정 source가 아니라 `owner[C]`에 도달해야 하며, 시작점·이웃 순서를 고정했다는 것은 하나의 동점 선택을 재현한다는 뜻이지 모든 최단 경로를 보존한다는 뜻은 아닙니다.

방향 그래프에서는 화살표 방향이 거리의 출발 방향을 결정합니다. 시설에서 정점으로 향하는 원래 간선을 따라 탐색하면 시설에서 각 정점까지의 거리이고, 정점에서 가장 가까운 시설까지의 거리가 필요하면 간선을 뒤집어 시설들을 거리 0으로 넣어야 합니다. 시작점이 없으면 모든 거리가 INF이고, 없는 정점 ID는 도달 불가가 아니라 입력 오류입니다.

다중 시작점에서는 `owner`가 거리 계산의 필수 값이 아니라 동점 경로를 설명하기 위한 부가 메타데이터임을 구분합니다. 무방향 그래프에서 A와 E를 동시에 넣은 뒤 C가 어느 source를 갖는지는 초기 큐 순서의 정책 결과이며 거리 2 자체는 변하지 않습니다. 방향을 뒤집어야 하는 문제를 놓치면 “가장 가까운 시설” 대신 “시설에서 각 정점까지의 거리”을 계산하게 되므로, 간선 방향을 작은 입력으로 먼저 대조합니다.
...
재현 시험은 유효한 source, 고립 정점, unknown source/target, self-loop, 평행 간선, 연결 요소가 섞인 그래프를 각각 둡니다. 예측 결과는 self-loop가 거리나 parent를 낮추지 않고, 평행 간선이 중복 enqueue를 만들지 않으며, unknown ID가 `INF`가 아니라 입력 오류로 반환되는 것입니다. 인접 리스트의 실제 이웃 열람 횟수도 세어 `O(V+E)`라는 가정이 구현에서 성립하는지 확인합니다.
