...
무방향 경로 A-B-C-D-E에서 A와 E를 시작점으로 두면 거리는 [0,1,2,1,0]입니다. C는 두 원천에서 동점이며 먼저 발견한 쪽을 owner로 남길 수 있습니다. 시작점·이웃 순서를 고정하면 재현할 수 있지만, 모든 동점 원천을 열거하는 것과는 다른 계약입니다. 부모 복원은 특정 고정 source가 아니라 owner까지 진행해야 합니다.

방향 그래프에서 원래 간선을 따라 여러 원천부터 탐색하면 '시설에서 정점까지'의 거리입니다. '정점에서 시설까지'가 필요하면 역방향 간선으로 다중 원천 탐색해야 합니다. 시작점이 없으면 모두 INF이며, 없는 정점 ID는 입력 오류로 처리합니다.
