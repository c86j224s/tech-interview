...
IOCP 수신 경로의 기본 상태 전이는 제출한 작업이 완료 포트에 어떤 형태로 돌아오는지와 그때까지 작업 메모리가 살아 있는지를 중심으로 읽습니다. 즉시 성공·pending·실패 완료·사용자 제어 패킷을 구분하고, 연결과 `OVERLAPPED` 컨텍스트의 소유 참조를 완료 시점까지 유지해야 다음 수신을 안전하게 제출할 수 있습니다.
...
검증용 상태표에는 즉시 성공, `WSA_IO_PENDING`, 완료 시 실패, 사용자 패킷, GQCS timeout을 별도 행으로 두고 각 행의 종결 소유자와 참조 감소 횟수를 기록하십시오. 예상 결과는 기본 통지 모드의 즉시 성공과 pending이 모두 완료 포트 경로로 끝나고, 통지 없는 확정적 즉시 실패만 제출 실패 경로에서 끝나는 것입니다. 실제 Windows 실행 전에는 이 문서의 API 발췌를 완성된 서버로 간주하지 않습니다.
