...
연결마다 수신을 하나만 걸면 수신 버퍼를 이어 붙이기 쉽습니다. 송신도 연결별 queue에서 한 작업만 진행하도록 시작하면, 앞 메시지의 미전송 부분을 처리한 뒤 다음 메시지를 보낼 수 있습니다. 그러나 수신 하나와 송신 하나는 동시에 완료될 수 있어 `closing`, 파서 상태, 송신 queue를 아무 워커나 무보호로 바꾸면 안 됩니다.

연결별 mutex를 쓰는 구현은 짧은 상태 변경과 제출·종료 경계를 잠금으로 묶습니다. 연결별 직렬 실행기를 쓰는 구현은 완료 결과를 그 실행기로 넘겨 순서를 정합니다. 어느 쪽이든 DB를 기다리는 동안 연결 잠금을 계속 잡거나, 후속 queue를 무한히 늘리는 방식은 피해야 합니다. 직렬화 대상을 전체 서버로 잡으면 서로 무관한 연결까지 기다립니다.
