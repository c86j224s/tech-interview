...
실시간 채널은 전달 경로이고 상태의 권위 저장소가 아닙니다. 연결이 끊겼을 때 복구하려면 서버가 이벤트를 얼마나 보관하는지, 클라이언트가 어디까지 적용했는지, 해당 위치 이후를 재생하거나 snapshot으로 대체할 수 있는지를 먼저 계약해야 합니다.
...
따라서 단순 알림·최신 상태에는 SSE가 운영상 단순할 수 있지만, 클라이언트 명령과 서버 응답을 같은 양방향 세션에서 다뤄야 하면 WebSocket을 검토합니다. 어느 채널을 선택해도 누락되면 안 되는 효과는 내구 로그와 멱등 적용 경계로 보호합니다.
...
유휴 장치 timeout보다 짧고 정상 RTT 변동을 견딜 간격·실패 임계값을 정합니다. 한 번 늦었다고 모두 재접속하면 복구 부하가 몰립니다. 지수 backoff·지터·최대 간격·접속 예산을 두고 실패한 연결과 이전 연결의 타이머를 정리합니다.
...
예상 결과는 `nextExpected=10`일 때 9번 이벤트는 중복으로 무시하고, 11번 이벤트는 gap으로 재생 또는 snapshot을 요청하며, 10번 적용과 커서 저장 사이 중단은 재적용되더라도 로컬 효과가 멱등이면 한 번만 남는 것입니다. snapshot 기준 커서 이후 이벤트를 보존하지 않으면 복구 완료를 선언할 수 없습니다.
