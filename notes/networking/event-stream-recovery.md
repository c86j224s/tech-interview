...
## 오래된 커서는 snapshot으로 전환합니다

서버 로그 시작이 100인데 클라이언트가 40까지만 적용했다면 41~99를 없었던 것으로 처리해서는 안 됩니다. 서버가 일관된 snapshot S와 그 기준 커서를 제공하고 S 이후 변경도 보존해야 합니다. snapshot 다운로드 중 새 이벤트가 오면 버퍼·재생 위치로 이어 붙입니다.

부분 snapshot을 완성 상태로 공개하지 않고 검증 후 교체합니다. 객체 삭제·세대 변경·권한 변경도 반영해야 합니다. 여러 기기의 커서를 하나로 합치면 한 기기의 ACK가 다른 기기의 미수신 데이터를 지울 수 있으므로 소비자 단위를 명확히 정합니다.

## 서버가 조금씩 써도 화면에는 모여 보일 수 있습니다
